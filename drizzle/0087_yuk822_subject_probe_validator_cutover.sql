-- YUK-822 cutover: retire every conjecture proposal that is still pending
-- before the subject-validator-bound probe_quality v4 producer starts.
--
-- This migration ships in the same image that first produces v4. The migrate
-- init container completes before app and worker startup, so every proposal
-- visible here predates the v4 runtime. Retiring the complete pending set avoids
-- stranding an old proposal behind the new fail-closed accept gate and frees its
-- cause x KC identity for regeneration.
--
...
SELECT
  'yuk822_subject_validator_retract_' || md5(proposal."id"),
  'agent',
  'yuk822_subject_validator_migration',
  'correct',
  'event',
  proposal."id",
  'success',
  jsonb_build_object(
    ...
    'reason_md', 'Conjecture retired at the YUK-822 subject-validator cutover; regenerate under probe-quality v4.',
