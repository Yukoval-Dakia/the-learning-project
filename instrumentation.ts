 ...
 *
 * pg-boss client init is NOT done here — webpack can't trace pg-boss's Node
 * built-ins through the instrumentation bundle (more restricted than route
 * bundles). The boss singleton is lazily started on first `boss.send(...)` in
 * a route via `getStartedBoss()` (see src/server/boss/client.ts).
 *
