...
- YUK-958 #1334：后端成功 quiz 显式 end，失败/取消/blocked 不结束；
  ...
  独立 review 和 scoped gates 绿色；UI 尚未修改，不能关闭整票。
- 当前main9302f08b重新实测依赖基线438/0/47（此前98f15bda为439/0/47）；
...
1. 完成 #1334 exact-head集成交付；不部署，不重复已完成的模型验收。
2. YUK-958：UI预检获批后，客户端消费显式结束状态，统一前台/后台消息投影，
