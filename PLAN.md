...
> **【更新 2026-07-11 · 地基批收口：YUK-599 + YUK-598 四 PR 全 merged】** 开工令第二阶段完成——**YUK-599**（#756 `b56ca263`：六表族 DDL + 两本 journal 共用 change_seq + trait-schemas/compose/seeds + 装配水合三层降级链〔journal 回溯 / 代码种子 / builtin 显式地板〕+ reconcile 单写者〔advisory lock + seed_version 短路 + owner-edited journal 谓词〕+ backup 4.14 六表 + restore setval；review APPROVE 4P3 折入，零行为变化主张被独立复跑证实）；**YUK-598 PR1**（#757 `34c4a2b5`：registry 三集合 API + entry meta + BUILTIN_IDS 改名 + KnownSubjectId→string + stale-const 三站收口 + GET /api/subjects〔server-only 负测〕+ useSubjects provider〔initialData 编译期投影〕+ 六消费文件行驱动含两处模块级冻结修复；review APPROVE 2P2+3P3 全折入）+ **PR2**（#758 `617f5683`：scope_key 三点收 RAW + 非幂等 id 回归；review 实证存量键全 slug-幂等 = 活键字面 no-op）。**每单独立 opus 对抗 review + 全量 gate + CI 绿后按 07-07 政策自主 merge**。⚠️ **未部署**：生产仍 = `91d39b6d`（Phase-0 窗），main 已含 0064 migration + 水合接线——批到下个部署窗口（宜与 YUK-600 同窗，migrate 会落 4 subject + 24 trait 种子行）。**当前刀口 = YUK-600**（thin-create 绑定式 + goal 防线 + knownSubjects + nightly gate + rubricGuidance 四锚点）。（上一条 Phase-0【更新】已滚存 → `docs/planning/2026-07-07-plan-header-log-archive.md`）
...
- **YUK-597 实施批 = 当前 active 线（开工令已下，Phase-0 已收口）**：科目 = 6 trait 绑定聚合 + opaque id `subj_<cuid2>`；权威依据 `docs/design/2026-07-10-yuk597-v3-trait-subjects.md`（**v3.2，owner 批准**；v2 doc 未被替换章节按 v3 头部明示继续生效）。依赖序（判词 C；YUK-598~602 描述已对齐 v3.2）：**Phase-0 ✅ 已上线**（YUK-610 #753 + YUK-611 #754，`91d39b6d` 已部署）→ **YUK-599 ✅**（#756）∥ **YUK-598 ✅**（#757+#758）——地基批 merged **未部署**（生产落后 4 merge，见【更新】）→ **当前 = YUK-600**（thin-create 绑定式 + goal 防线 + knownSubjects + nightly gate + rubricGuidance 四锚点）→ **YUK-602（提前）**（onboarding 手填 UI，owner 原话落点）→ **YUK-601**（trait 编辑面 + §3.5 读面 + 漂移治理 + 夜间 cron --strict）。⚠️ YUK-601/602 动 UI 前须先落一份合并 UI design doc 获批（UI Design Compliance）。PR-0 = YUK-603 已收口上线（见 ✅）。
...
- **无 PR 在飞**（2026-07-11 凌晨批：#756 YUK-599 `b56ca263` · #757/#758 YUK-598 `34c4a2b5`/`617f5683` 全部 merged）。**生产 = `91d39b6d`（Phase-0 窗），落后 4 merge——0064 migration + 水合接线未部署，批到下窗（宜与 YUK-600 同窗）**。worktree 拓扑 = main + `tlp-deploy`（`91d39b6d`）+ `tlp-yuk599`（实施 lane 工作树，现推 YUK-600）。**待 owner 清理分支**（squash-merge 后 guard 挡强删，均本地）：`yuk-249-yuwen-migration` · `yuk-249-rename-stragglers`（+远端）· `yuk-597-v3-trait-contract-docs` · `yuk-598-registry-provider` · `yuk-598-scope-key` · `yuk-599-trait-tables`。
