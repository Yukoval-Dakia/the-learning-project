...
> Linear 是权威 tracker；更新于 2026-09-26：**1038 主线 16/17 merged**（1046/1044/1048/1043/1050/1045/1047/1041/1055/1052/1049/1056/1051/1053/1054/1056-ops 已落地；**1057** rehearsal+D18 harness #1482 在 CI 等绿）。**backfill 双波审计** 共抓 **35 条漏裁 P1**（ora-4 两波，#1465–#1482 六+五张已 merge PR 全零回复/零修复）；拆 10 张 corrective 票 **YUK-1091–1100**，8 条 lane 全部交付——已 merge **1091 #1489 / 1092 #1487 / 1094 #1484 / 1095 #1483 / 1098 #1485 / 1100 #1488**（6/8），在 CI **1093 #1490 / 1099 #1491 / 1097 #1492**。owner 三连裁决已落地：①**1099 选 B**（legacy `reference_md` 文本即真答案，AI upgrade 提答质量后置）；②**advisory 评审合并等待窗 ~17min**（最后一次 push 后至少等约 17 min 再 merge，窗口内出现的 P0/P1 必须裁决才能合）；③**re-answer/effective-head 为既有设计**（head 指向最后 activated evaluation，非最新 submission —— grounding §11 L376-402，1481 方向正确）。**owner 热加载指令**：`全部做到 hot reload，硬编码改成加载配置`——1007 产品目标从「读面板」升级为「运行时配置中心」（DB config table + 进程内 cache + 版本失效 + 读方 per-call 活读），hot-reload 架构 grounding 已落盘 `docs/planning/2026-09-26-yuk1007-hot-reload-config.md`（imp-32：Tier A≈26 组~40 运行时旋钮直接换 getConfig，Tier B≈8 组模块加载快照待函数化，Tier C schema/cron/secret 不热）。生产 `RESEARCH_MEETING_AGENT_ENABLED` 已翻真（Q-572 A）。**infra 教训**（沉淀进记忆）：① macOS TCC 会按进程链剥夺 `git`/`gh` 对外置卷 I/O——`Operation not permitted` 先重试（可自愈），否则 System Settings 里给承载 app（Ghostty 或 opencode 宿主）补「可移动卷」；② gh CLI `run view --job`/`api .../logs` 在 job-logs 303→Azure 重定向上会 flake，用 `curl -sL --http1.1 -H "Authorization: token $(gh auth token)" https://api.github.com/repos/.../actions/jobs/<id>/logs` 直取；③ OpenCode V2 外目录写权限走 `permissions[]` 的 `external_directory` 规则（V1 `edit.allow` 管不到 subagent 写）——用户配 `~/.config/opencode/opencode.jsonc` 已落 `resource: tlp-wt-*/* allow`。**hooks**：dangerous-git guard 禁 force push / `worktree remove --force` / `branch -D` 上游 main 分支；review budget ≤ 2 轮 oracle，第 3 轮起直接拍回 imp。**PLAN 四栏常态**：每收尾一 lane 立刻更此头部+NOW/进行中/待拍板/阻塞。下一步待 merge：1057（rehearsal 收尾）→ 1058（invalidation 失效重放）→ 1059（release integration）。改正波 10/10 出票齐：1091/1092/1093/1094/1095/1096（五张已合）+ 1097（DB 触发器 #1492）+ 1098（capture 红act #1485 已合）+ 1099（publish 漂移 #1491）+ 1100（apply 安全 #1488 已合）。


## NOW

- 09-26 **YUK-1038 波次 5 lane + 8-lane 审计纠偏**：wave-2 五 lane（1049 Jev / 1051 UI / 1052 提交 / 1053 结算 / 1056 常量）全交付；**backfill 双波 35 P1** 拆 10 票纠正（6 合 4 在 CI）。
