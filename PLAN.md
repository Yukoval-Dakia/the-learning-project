...
> Linear 是权威 tracker；更新于 2026-09-26：**pstack 全自动 ship 能力落地**——clone `cursor/plugins@ecc249f` 至 `.slim/clonedeps/repos/`（MIT，仅 `pstack/` 子目录）；新 skill：`ship-mode`（无 spec 目标→playbook 路由+证据契约）、`verify-app`（dev harness 控制面：5173 UI/8787 API/worker）、`swarm`（并行 worker fan-out+聚合）、`issue-to-pr`（issue→复现→修→draft PR fail-closed 外环）、`worklog`（TSV 决策日志）；值班层 `.omc/ship-engine/bin/ship-tick.sh` + `ship-loop.sh`（repo 外、gitignored，自定位）：tick = 盘点→babysit（conflicts>threads>CI 序；独立 verifier verdict 门控 merge，ledger 按 PR+headSHA、patch-id 复用）→认领 ready-for-agent→owner lane 全生命周期；liveness 只读探活 + zombie reconcile + retry-by-mode + terminal handoff。审计 `.remember/worklog-ship-engine.tsv` + `.omc/ship-engine/{units,ledger}.tsv`：tick 盘点在途 lane→babysit PR（CI 绿+无未裁决 P0/P1 → squash merge，红 → 唤醒 lane session 修，2 次同因失败 blocked）→ `ready-for-agent` 认领新票→worktree lane session 实现到 PR；AGENTS.md 已写入证据契约（委派必交 artifact、不接受自报、wrong-surface/INCONCLUSIVE 不算过）。前一更新——2026-09-24：sweep-4/5 派生 follow-up 批清完——1031/1032/1034/1035/1036 五 merge，1033 owner 裁决暂缓（等 1007 面板）；审计与评审派生 YUK-1037（verify-enroll 合成根 FSRS 卡 bug）/1038（生成链 reference pair 缺口）。本批未部署，生产仍为 `c89079b68`。
