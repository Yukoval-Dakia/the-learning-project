...
import { upsertFsrsState, getFsrsState } from '@/server/fsrs/state';
...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    // Read prior FSRS state (may be null for first review). Schedule via ts-fsrs.
    let prevStateRow: Awaited<ReturnType<typeof getFsrsState>> = null;
    let result: ReturnType<typeof scheduleReview>;
    try {
      prevStateRow = await getFsrsState(db, 'question', questionId);
      result = scheduleReview(
        prevStateRow?.state
          ? {
              ...prevStateRow.state,
              // last_review is z.coerce.date()-typed; the value on disk is Date
    ...
    // again → failure, hard|good → success (Lane B invariant)
    const outcome: 'success' | 'failure' = body.rating === 'again' ? 'failure' : 'success';
    const eventId = newId();
    const fsrsStateAfter = {
      ...result.nextState,
      // Coerce Date | null for parseEvent (which expects ISO/date types either way).
