...
export async function POST(
  ...
  try {
    ...
    for (const block of body.blocks) {
      ...
      // Attempt event (failure) — replaces the legacy mistake row. The
      // attempt event id doubles as the back-compat `mistake_id` returned to
      // clients (opaque token; semantics shifted post-Step-9).
      ...
      // `imported_mistake_id` column kept as nullable text (the column itself
      // is not dropped — only the mistake table is). Point at the attempt
      // event id so the existing block↔mistake link doesn't break clients
