...
`pnpm audit:architecture-deepening`（YUK-886 / F4.1）调用 `scripts/audit-architecture-deepening.ts`，是合并了 YUK-885 architecture-ownership 审计的**定量架构审计**：51 个 owned TaskSpec（ProfileCriticTask 属 ingestion 且 `compile-profile --critic` 在册）/ 0 中央语义定义、每个注册 DomainTool 单 owner + 输入输出 schema + manifest/registry/allowlist 对账 + MCP 输出校验 / 0 中央 concrete 工具、每个域 pg-boss 队列 manifest-owned 且注册元数据对账 / 0 中央语义注册、每个受支持 proposal kind 单 owner 且 accept/dismiss/retract 声明齐全 / 0 中央 kind-switch、中央 events 仅 transport/envelope、Notes/Memory recovery 与 verify dispatch conformance 测试在册、provider attempt census 全分类（无 unknown-as-zero）、依赖总量严格低于 531/70/62 且 deep/隐藏写/未编目回环全零、原始 SCC 完全由 `scripts/capability-public-read-cycles.ts` 有界目录解释。红测试在 `scripts/audit-architecture-deepening.unit.test.ts`（16 类 fixture）。它接入 `pnpm test` 链与 CI Audits 步骤。`audit:capability-boundaries` 的 baseline 与本审计的 531/70/62 天花板是两道独立 gate，不得为过其一而放宽另一。
