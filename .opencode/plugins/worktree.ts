...
import { processPendingDelete } from "./worktree/delete"
import {
...
import { type TerminalResult, openTerminal } from "./worktree/terminal"
...
/** Run pre-delete hooks as a fail-closed gate. */
async function runHooksFailClosed(
	cwd: string,
	commands: string[],
	log: Logger,
): Promise<Result<string, string>> {
	for (const command of commands) {
		log.info(`[worktree] Running pre-delete hook: ${command}`)
		try {
			const result = Bun.spawnSync(["bash", "-c", command], {
				cwd,
				stdout: "inherit",
		...
		tool: {
			...
			worktree_delete: tool({
				...
				async execute(_args, toolCtx) {
					...
					const reservation = reservePendingDelete(
						database,
						{ branch: session.branch, path: session.path },
						client,
					)
					if (!reservation.accepted) {
						const pendingBranch = reservation.pending?.branch ?? "another worktree operation"
		...
		event: async ({ event }: { event: Event }): Promise<void> => {
			...
			const deleteResult = await processPendingDelete(pendingDelete, {
				...
				runPreDeleteHooks: async (worktreePath) => {
					...
					return runHooksFailClosed(worktreePath, config.hooks.preDelete, log)
				},
				completeState: (expectedPendingDelete) =>
					completePendingDelete(database, expectedPendingDelete),
			})
			if (!deleteResult.ok) {
				log.error(
					`[worktree] Cleanup blocked at ${deleteResult.stage}: ${deleteResult.error} Pending/session state was preserved.`,
				)
				return
			}
