...
/** Result of reserving the singleton pending-operation slot for deletion. */
export type PendingDeleteReservation =
	| { readonly accepted: true; readonly pending: PendingDelete }
	| { readonly accepted: false; readonly pending: PendingDelete | null }

/**
 * Atomically reserve the pending-operation slot for a delete.
 *
 * Unlike the legacy last-write-wins behavior, this never replaces another
 * pending spawn/delete. That keeps concurrent cleanup requests from orphaning
 * the first worktree and gives the caller an explicit conflict to report.
 */
export function reservePendingDelete(
	db: Database,
	del: PendingDelete,
	...
	const stmt = db.prepare(`
		INSERT OR IGNORE INTO pending_operations (id, type, branch, path, session_id)
	...
	if (inserted.changes === 1) {
		return { accepted: true, pending: parsed }
	}

	const existingDelete = getPendingDelete(db)
	const existingSpawn = getPendingSpawn(db)
	const existingLabel = existingDelete?.branch ?? existingSpawn?.branch ?? "unknown"
	logWarn(
		client,
		"worktree",
		`Refused pending delete for "${parsed.branch}"; another operation is already pending for "${existingLabel}"`,
	)
	return { accepted: false, pending: existingDelete }
}
...
/**
 * Atomically clear the exact pending delete and its matching session rows.
 * Returns false without deleting sessions if the pending operation changed.
 */
export function completePendingDelete(db: Database, pending: PendingDelete): boolean {
	const parsed = pendingDeleteSchema.parse(pending)
	const complete = db.transaction(() => {
		const deletedPending = db
			.prepare(`
				DELETE FROM pending_operations
				WHERE id = 1 AND type = 'delete' AND branch = $branch AND path = $path
			`)
			.run({ $branch: parsed.branch, $path: parsed.path })
		if (deletedPending.changes !== 1) return false

		db.prepare("DELETE FROM sessions WHERE branch = $branch AND path = $path").run({
			$branch: parsed.branch,
			$path: parsed.path,
		})
		return true
