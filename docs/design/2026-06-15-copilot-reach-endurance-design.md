...
**reach gap**: registry 共 **36 个 task kind**（`src/ai/registry.ts`），copilot 直接够得着 **26 个 copilotTools**（`src/server/ai/tools/allowlists.ts:136` 五包 manifest union；YUK-362 纠正旧 stale 写 25）。重活够不着或只能 inline 嵌套:`proposal-tools.ts:122-123`（`runTask` inline）、`runVariantGen`/`runQuestionAuthor`（1687/1776）。
...
**数量真相与「大部分是 fanout」相反**: 26 copilotTools 里 ~15 是读、fanout 写**只有 3**（且 variant/attribute 仅 chip surface）、直接结构化写 ~8、inline-生成写 2。fanout 是少数,只留给真正需要专门生成 pass 的（出题/变式/错因判断）。
...
模型收敛:reach=生成型(run_task dispatcher)/结构型(typed apply) 双汇单 applier;endurance=短 inline/长 durable job/summarize-continue,durable-run 形态已 grounded 落 §3.4(run handle=checkpoint_id、复用 job_events 进度流、run card、interrupt/queue 串行化);checkpoint 在单 applier 点戳事件。**三腿已 ADR 化 → ADR-0041（决策方向 ratified）**。实施细节 deferred(不阻 ADR):intentSchema/prepare 契约的 registry 落地形、reply_delta 持久化粒度分叉(§5.5)、sync/async 阈值实测——见 ADR-0041「实施细节 deferred」节。（原列「worker copilotTools 接线补全前置」已删——YUK-362 证它非前置，见 §5.3 修正。）
