...
## 2. Architectural truth (READ ADR-0006 v2 + ADR-0010 FIRST)

The system is **event-driven** with a **knowledge mesh**. Every action — by user, by AI agent, by cron — is a row in the `event` table. Knowledge nodes are linked via tree backbone + typed mesh edges. **Tree is the skeleton, mesh is the muscle.**

```
[material]                   question / knowledge / source_document / artifact
                             (artifact is where AI's generated content lands)
                             knowledge has parent_id (tree) + knowledge_edge table
                             (mesh: prerequisite / related_to / contrasts_with /
                              applied_in / derived_from + experimental:*)
...
[event]   ★ THE CORE ★       actor_kind × action × subject_kind 三轴
                             ...
                             - subject_kind: 'question' | 'knowledge' | 'knowledge_edge'
                                         | 'artifact' | 'source_document' | 'event' (chain)
...
| `/knowledge` | 知识 tree backbone + **mesh edges** + AI 提议入口（节点 / 边都可被 AI 提议）| 新：每节点显示该节点的 mesh 关系（prerequisite / related_to / contrasts_with / applied_in / derived_from）+ 该节点上 AI 最近活动（events filtered by knowledge_id）。**Force-directed graph 视图必须**（per ADR-0010；不再是 §9 open question）|
...
### 6.4 Edge proposal review（mesh C 档新增）
AI 在 Dreaming / KnowledgeReviewTask 中提议 `knowledge_edge`（"[实词词义] —prerequisite→ [翻译]，AI 推理：用户最近翻译错有 70% 涉及实词词义"）。UI 呈现：

- 在 `/today` proposal inbox + `/knowledge` 节点页内嵌"AI 建议的相关节点"区
- 接受 / 拒绝 / 修改方向 / 改关系类型四个动作 → 写 `event(action='rate', subject='knowledge_edge')`
- 拒绝过的不再重复提议（AI 记忆通过 rate 历史推断）

形态待 grill：**节点页内嵌 vs 集中 inbox**（与 §6.2 提议 inbox 选项 B/C 关联）。

### 6.5 Cost transparency（C 档信任建立）
AI 大量产出意味着用户要信任成本可控：
...
3. **`docs/adr/0010-knowledge-mesh.md`** ← mesh = tree backbone + typed edges（必读）
4. **`docs/adr/0008-learning-session-multi-type-envelope.md`** ← session 多 type + conversation 吸收 agent_sessions
5. **`CONTEXT.md`** §已批准 v2 ← 词条 + 概念映射表
6. **`docs/design/loom-design/project/README.md`** §Content + §Visual Foundations ← 声调 / 视觉规范（参考，**不读 §Sources 或 §Index 里关于 9 admin pages 的描述，那是旧的**）
7. **`docs/design/loom-design/project/colors_and_type.css`** ← design tokens（直接 port）
...
7. ~~Knowledge graph 的可视化~~ —— **已决（ADR-0010 mesh 落定后必须）**：force-directed graph 视图，按 relation_type 分色；table 仍作默认 + toggle 切 graph。< 1000 节点 / ~5000 edges 规模设计。具体 graph lib（D3 / cytoscape / vis-network）由设计者根据交互需求提议
8. **Edge proposal review** UX：节点页内嵌 vs 集中 inbox（§6.4 + §6.2 关联）
