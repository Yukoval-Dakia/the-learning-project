...
const TodayScreen = ({ db, nav, setProposalStatus, proposalStatus, lanesMode }) => {
  ...
  // v2.1 §1.3: replaced "AI 提议 · 24h" with "AI 提议 · 待审" — pending only, click → /inbox
  const pendingProposals = db.events.filter(e =>
    e.actor_kind === 'agent' &&
    (e.action === 'propose' || (e.action === 'generate' && (e.subject_kind === 'artifact' || e.subject_kind === 'knowledge_edge'))) &&
    !proposalStatus[e.id]
  );
  ...
  const dreamDurMin = ((dreamSession.ended_at - dreamSession.started_at) / 60).toFixed(0);

  // Counts for inbox strip
  const dreamVariants  = dreamProposals.filter(e => e.action === 'generate' && e.payload.artifact_kind === 'variant').length;
  const dreamNotes     = dreamProposals.filter(e => e.action === 'generate' && e.payload.artifact_kind === 'note').length;
  const dreamQuiz      = dreamProposals.filter(e => e.action === 'generate' && e.payload.artifact_kind === 'quiz').length;
  const dreamNewNodes  = dreamProposals.filter(e => e.action === 'propose' && e.subject_kind === 'knowledge').length;
  const dreamNewEdges  = dreamProposals.filter(e => e.action === 'propose' && e.subject_kind === 'knowledge_edge').length;
  ...
  return (
    <main className="page wide">
      ...
      <div className="kpi-strip">
        ...
        <button className="kpi kpi-clickable" type="button" onClick={() => nav('inbox')}>
          <div className="kpi-label">AI 提议 · 待审</div>
          <div className="kpi-num">{pendingProposals.length}<small> 条</small></div>
          <div className="kpi-trend up">→ /inbox</div>
        </button>
        <div className="kpi">
      ...
      {dreamProposals.length > 0 && (
        <div className="inbox-strip">
          <div className="inbox-text">
            <div className="src"><ActorBadge actorKind="cron" actorRef="nightly_dreaming" /> · 跑了 {dreamDurMin} 分钟 · ${dreamCost.toFixed(3)} · {relTime(dreamSession.started_at)}启动</div>
            <h3>昨晚 AI 提议了 {dreamProposals.length} 条，要看吗？</h3>
            <div className="breakdown">
              {dreamVariants > 0 && <div><b>{dreamVariants}</b><span>变式</span></div>}
              {dreamNotes > 0    && <div><b>{dreamNotes}</b><span>笔记</span></div>}
              {dreamQuiz > 0     && <div><b>{dreamQuiz}</b><span>小测</span></div>}
              {dreamNewNodes > 0 && <div><b>{dreamNewNodes}</b><span>新知识点</span></div>}
              {dreamNewEdges > 0 && <div><b>{dreamNewEdges}</b><span>关系建议</span></div>}
            </div>
          </div>
          <div className="row gap-md">
            <Button variant="secondary" onClick={() => nav('mistakes')}>分散审批</Button>
            <Button variant="primary" iconRight="arrowR" onClick={() => nav('inbox')}>集中审批 (全部 {pendingProposals.length})</Button>
          </div>
        </div>
      )}
...
// ════════════════════════════════════════════════════════════
const MistakesScreen = ({ db, nav, proposalStatus, setProposalStatus, chainRowCost }) => {
  const { byId, childrenOf } = useMemo(() => indexEvents(db.events), [db.events]);
            ...
            <li key={att.id} className="event-card">
              ...
              {judge && (
                <div className="ec-judge">
                  ...
                  <EventChain eventId={judge.id} eventsById={byId} label="完整推理链" chainRowCost={chainRowCost} />
                </div>
              )}
...
// ════════════════════════════════════════════════════════════
const ReviewScreen = ({ db, nav, chainRowCost }) => {
      ...
      <section className="review-stage">
        ...
        {judge && (
          <>
            ...
            <EventChain eventId={judge.id} eventsById={byId} label="查看 AI 推理（3 events）" chainRowCost={chainRowCost} />
          </>
        )}
        ...
        <>
          ...
          <div className="quality-strip">
            ...
            <div className="rescue">
              <button className="tier-btn tier-manual" type="button">
                <span className="tier-label"><Icon name="pen" size={14} /> 手动改</span>
                <span className="tier-meta">本地 · 免费</span>
              </button>
              <button className="tier-btn tier-2" type="button">
                <span className="tier-label"><Icon name="zap" size={14} /> Vision Tier 2</span>
                <span className="tier-meta">Haiku · ~$0.005</span>
              </button>
              <button className="tier-btn tier-3" type="button">
                <span className="tier-label"><Icon name="brain" size={14} /> Vision Tier 3</span>
                <span className="tier-meta">Sonnet · ~$0.18</span>
              </button>
            </div>
          </div>
...
// ════════════════════════════════════════════════════════════
//   /knowledge — tree + mesh + force-directed graph
//   v2.1: relations panel in detail drawer; edge proposals first-class.
// ════════════════════════════════════════════════════════════
const KnowledgeScreen = ({ db, nav, proposalStatus, setProposalStatus, chainRowCost }) => {
  const { byId } = useMemo(() => indexEvents(db.events), [db.events]);
  const [view, setView] = useState('tree'); // 'tree' | 'graph'
  const [selectedId, setSelectedId] = useState(null);

  // node activity index — events with subject_kind=knowledge for this id
  // OR events referencing this id in payload.referenced_knowledge_ids
  const nodeActivity = useMemo(() => {
    ...
    for (const ev of db.events) {
      ...
      // edges also live on a node
      if (ev.subject_kind === 'knowledge_edge') {
        if (ev.payload?.from_id) refs.add(ev.payload.from_id);
        if (ev.payload?.to_id)   refs.add(ev.payload.to_id);
      }
  ...
  // attempts (mistakes) per knowledge — used to size graph nodes
  const mistakeCounts = useMemo(() => {
    const m = {};
    for (const ev of db.events) {
      if (ev.action !== 'attempt' || ev.outcome !== 'failure') continue;
      const q = db.questions[ev.subject_id];
      if (!q) continue;
    ...
    return m;
  }, [db.events, db.questions]);

  // pending knowledge proposals (nodes) — keep per-parent
  const nodeProposals = db.events
  ...
  // pending edge proposals — group by "either end is this node"
  const edgeProposals = db.events
    .filter(e => e.action === 'propose' && e.subject_kind === 'knowledge_edge' && !proposalStatus[e.id]);
  const edgesPendingAt = (kid) =>
    edgeProposals.filter(e => e.payload.from_id === kid || e.payload.to_id === kid);

  const selected = db.knowledge.find(k => k.id === selectedId);
  const selectedEdges = (db.edges || []).filter(e =>
    !e.archived_at && (e.from_id === selectedId || e.to_id === selectedId)
  );
  const selectedPendingEdges = selectedId ? edgesPendingAt(selectedId) : [];
  const selectedActivity = (nodeActivity[selectedId]?.recent || [])
    .sort((a, b) => b.created_at - a.created_at)
    .slice(0, 8);
  ...
  return (
    <main className="page wide">
      <PageHeader
        eyebrow={`KNOWLEDGE · ${db.knowledge.length} nodes · ${(db.edges || []).length} edges (mesh)`}
        title="知识"
        sub="树是骨架，mesh 是肌肉。点节点 → 右侧抽屉看关系 + 活动 + AI 提议。">
        <div className="kg-toggle">
          <button className={view === 'tree' ? 'is-on' : ''} onClick={() => setView('tree')}>树</button>
          <button className={view === 'graph' ? 'is-on' : ''} onClick={() => setView('graph')}>Graph</button>
        </div>
        <Button variant="primary" icon="plus">新建节点</Button>
      </PageHeader>

      {/* Distributed-mode pending edge proposals strip (count) */}
      {edgeProposals.length > 0 && (
        <div className="quality-strip" style={{ background: 'var(--coral-soft)', border: '1px solid var(--coral-line)', color: 'var(--coral-ink)', marginBottom: 'var(--s-5)' }}>
          <Icon name="link" size={20} />
          <div className="qtxt">
            <h5 style={{ color: 'var(--coral-ink)' }}>
              AI 提议了 {edgeProposals.length} 条新关系
            </h5>
            <p style={{ color: 'var(--coral-ink)' }}>
              来自昨晚 Dreaming + Maintenance · 节点选中后右侧抽屉里 accept/reverse/change/dismiss。或集中审批 →
            </p>
          </div>
          <div className="rescue">
            <Button variant="primary" size="sm" iconRight="arrowR" onClick={() => nav('inbox')}>集中审批</Button>
          </div>
        </div>
      )}
      ...
      {view === 'tree' && (
        <div className="tree">
          {db.knowledge.filter(k => !k.proposed).map(k => {
            ...
            const hasNodeProposal = !!proposalsByParent[k.id];
            const hasEdgeProposal = edgesPendingAt(k.id).length > 0;
            return (
              <React.Fragment key={k.id}>
                <button type="button"
                  className={`tree-node ${hasNodeProposal || hasEdgeProposal ? 'has-proposal' : ''} ${k.id === selectedId ? 'is-selected' : ''}`}
                  data-depth={k.depth}
                  ...
                  <div className="activity">
                    {mistakeCounts[k.id] ? <span>{mistakeCounts[k.id]} 错</span> : null}
                    {acts.length > 0 && <span>· {acts.length} 事件</span>}
                    {acts.filter(e => e.actor_kind === 'agent').length > 0 && (
                      <ActorBadge actorKind="agent" compact />
                    )}
                    {hasEdgeProposal && <Badge tone="coral">新关系</Badge>}
                  </div>
                  <div className="actions">
                    <span className="meta-mono" style={{ fontSize: 11, color: 'var(--ink-4)' }}>→</span>
                  </div>
                </button>
              </React.Fragment>
            );
      ...
      {view === 'graph' && (
        <>
          <div className="kg-stage">
            <KnowledgeGraph
              nodes={db.knowledge.filter(k => !k.proposed)}
              edges={(db.edges || []).filter(e => !e.archived_at)}
              selectedId={selectedId}
              onSelect={setSelectedId}
              mistakeCounts={mistakeCounts}
              width={1000} height={520} />
            <div className="kg-legend">
              <span className="item" key="__tree">
                <span className="swatch" style={{ borderTopColor: 'var(--ink-5)', borderTopStyle: 'dashed' }} />
                <span>tree (parent_id)</span>
              </span>
              {Object.entries(RELATION_TYPES).map(([rt, r]) => {
                const c = ({
                ...
                return (
                  <span className="item" key={rt}>
                    <span className="swatch" style={{ borderTopColor: c, borderTopStyle: rt === 'related_to' ? 'dashed' : 'solid' }} />
                    <span>{r.label} ({rt})</span>
                  </span>
                );
              })}
              <span style={{ marginLeft: 'auto' }}>圆 = 节点 · 半径 ∝ mistake_count · 点圆选中节点</span>
            </div>
          </div>
        </>
      )}

      {/* Detail drawer (lifted EventChain too — v2.1 §1.1: chains live in drawer here, not inline under tree) */}
      {selected && (
        <aside className="detail-drawer">
          <header className="dd-head">
            <div>
              <h3>{selected.name}</h3>
              <div className="meta"><code>{selected.id}</code> · depth {selected.depth}{selected.parent_id ? ' · parent ' + selected.parent_id : ''}</div>
            </div>
            <Button variant="ghost" icon="x" onClick={() => setSelectedId(null)} aria-label="关闭" />
          </header>
          <div className="dd-body">
            {/* Edge proposals first — they're the action items */}
            {selectedPendingEdges.length > 0 && (
              <section className="dd-section">
                <h4>AI 建议关系 · {selectedPendingEdges.length} 待审</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {selectedPendingEdges.map(ev => (
                    <EdgeProposalCard
                      key={ev.id}
                      event={ev}
                      eventsById={byId}
                      ...
                      chainRowCost={chainRowCost}
                      onAccept={(e) => setProposalStatus(s => ({ ...s, [e.id]: 'accept' }))}
                      onReverse={(e) => setProposalStatus(s => ({ ...s, [e.id]: 'reverse' }))}
                      onChangeType={(e) => setProposalStatus(s => ({ ...s, [e.id]: 'change_type' }))}
                      onDismiss={(e) => setProposalStatus(s => ({ ...s, [e.id]: 'dismiss' }))}
                    />
                  ))}
                </div>
            ...
            {/* §2.3.a — Tree section (parent + children from parent_id) */}
            <section className="dd-section">
              <h4>层级 · tree</h4>
              <div className="relations-list">
                {selected.parent_id ? (
                  <div className="relation tone-neutral">
                    ...
                    <span className="rel-target">{db.knowledge.find(k => k.id === selected.parent_id)?.name || selected.parent_id}</span>
                  </div>
                ) : (
                  <div className="empty-tiny">根节点</div>
                )}
                {db.knowledge.filter(k => k.parent_id === selectedId && !k.proposed).map(child => (
            ...
            {/* §2.3.a — Mesh section (cross-edges only — never duplicates tree) */}
            <section className="dd-section">
              <h4>关系 · mesh · {selectedEdges.length}</h4>
              {selectedEdges.length === 0 && <div className="empty-tiny">尚无 mesh 边。tree backbone 之外没有横向链接。</div>}
              <div className="relations-list">
                {/* Group by direction */}
                {selectedEdges.length > 0 && (
                  <>
                    {Object.keys(RELATION_TYPES).map(rt => {
                      const group = selectedEdges.filter(e => e.relation_type === rt);
                      if (!group.length) return null;
                      return (
                        <div className="relations-group" key={rt}>
                          {group.map(edge => (
                            <KnowledgeRelation key={edge.id} edge={edge}
                              currentNodeId={selectedId}
                              knowledge={db.knowledge}
                              onArchive={(e) => {/* would write rate=dismiss */}} />
                          ))}
                        </div>
                      );
                    })}
            ...
            {/* Node-level node proposals */}
            {proposalsByParent[selectedId]?.length > 0 && (
              <section className="dd-section">
                <h4>AI 建议子节点 · {proposalsByParent[selectedId].length}</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {proposalsByParent[selectedId].map(p => (
                    <div className="proposal" key={p.id}>
                      <div className="proposal-head">
                        <Badge tone="info" dot dotStatic>
                          <Icon name="plus" size={11} /> AI · 新节点
                        </Badge>
                        <span className="title">{p.payload.name}</span>
                      </div>
                      <div className="body" style={{ fontFamily: 'var(--font-sans)' }}>{p.payload.reasoning}</div>
                      <div className="proposal-actions">
                        ...
                        <span className="spacer" />
                        <EventChain eventId={p.id} eventsById={byId} label="为什么提议" chainRowCost={chainRowCost} />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Recent activity (AI + user) */}
            <section className="dd-section">
              <h4>近活动 · {selectedActivity.length}</h4>
              {selectedActivity.length === 0 && <div className="empty-tiny">无近期事件</div>}
              <div className="dd-activity">
                {selectedActivity.map(ev => (
                  <div className="dd-activity-row" key={ev.id}>
                    <div className="top">
                      <ActorBadge actorKind={ev.actor_kind} actorRef={ev.actor_ref} compact />
                      <span>{ACTION_LABEL[ev.action] || ev.action}</span>
                      <span>·</span>
                      <code>{ev.id}</code>
                      <span style={{ marginLeft: 'auto' }}>{relTime(ev.created_at)}</span>
                    </div>
                    <div className="body">{describeEvent(ev)}</div>
                  </div>
                ))}
              </div>
            </section>
...
// ════════════════════════════════════════════════════════════
const InboxScreen = ({ db, nav, proposalStatus, setProposalStatus, chainRowCost }) => {
  const { byId } = useMemo(() => indexEvents(db.events), [db.events]);
  const proposals = db.events
    .filter(e =>
      ((e.action === 'propose') ||
       (e.action === 'generate' && e.actor_kind === 'agent' && e.subject_kind === 'artifact')) &&
      !proposalStatus[e.id] &&
  ...
  // Split: edges get a dedicated section since they need different actions (reverse/change)
  const edgeProposals = proposals.filter(e => e.subject_kind === 'knowledge_edge');
  const nodeProposals = proposals.filter(e => e.subject_kind === 'knowledge');
  const artifactProposals = proposals.filter(e => e.subject_kind === 'artifact');
  ...
  return (
    <main className="page">
      ...
      {edgeProposals.length > 0 && (
        <>
          <div className="section">
            <h2>关系建议 · {edgeProposals.length}</h2>
            <div className="meta">subject_kind=knowledge_edge · ADR-0010 mesh</div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {edgeProposals.map(ev => (
              <EdgeProposalCard
                key={ev.id}
                event={ev}
                eventsById={byId}
      ...
      {nodeProposals.length > 0 && (
        <>
          <div className="section">
            <h2>新节点 · {nodeProposals.length}</h2>
            <div className="meta">subject_kind=knowledge · 加到 tree backbone</div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {nodeProposals.map(ev => (
              <ProposalCard
                key={ev.id}
                event={ev}
                eventsById={byId}
      ...
      {artifactProposals.length > 0 && (
        <>
          <div className="section">
            <h2>内容生成 · {artifactProposals.length}</h2>
            <div className="meta">action=generate subject_kind=artifact · 变式 / 笔记 / 小测 / 总结</div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {artifactProposals.map(ev => (
              <ProposalCard
                key={ev.id}
                event={ev}
                eventsById={byId}
...
const CopilotDrawer = ({ db, onClose, contextRoute, contextEntity, setProposalStatus, nav, toolUseDetail = 'folded' }) => {
  ...
  // §1.6 #3b — chip click writes an `accept_suggestion` event (not a fake
  // user ask), then runs the tool. UI renders the accept as a chip-tag row
  // rather than a chat bubble.
  const acceptSuggestion = (action) => {
    const acceptEv = {
      id: `ec_ac_${Date.now()}`, session_id: 's_copilot',
      actor_kind: 'user', actor_ref: 'self',
      action: 'experimental:accept_suggestion',
      subject_kind: 'event', subject_id: feed.length ? feed[feed.length - 1].id : 's_copilot',
      payload: { label: action.label, tool: action.tool, args: action.args },
      caused_by_event_id: feed.length ? feed[feed.length - 1].id : null,
      created_at: Math.floor(Date.now() / 1000),
    };
    setFeed(f => [...f, acceptEv]);
    setTyping(true);
    // Simulate the tool firing — for the prototype, fall through to scripted
    // tool_use + explain. Real backend would dispatch the named tool.
    runAgentToolCall(action, acceptEv.id);
  };

  const runAgentToolCall = async (action, causedById) => {
    await new Promise(r => setTimeout(r, 700));
    // Mocked tool result: pretend the tool succeeded with a stub summary.
    const toolEv = {
      id: `ec_tu_${Date.now()}`, session_id: 's_copilot',
      actor_kind: 'agent', actor_ref: 'copilot',
      action: 'experimental:tool_use',
      subject_kind: 'query', subject_id: `tu_live_${Date.now()}`, outcome: 'success',
      payload: {
        tool: action.tool,
        args: action.args || {},
        result_summary: `${Math.floor(Math.random() * 3) + 1} result(s) (live)`,
        result_data: [],
      },
      caused_by_event_id: causedById,
      task_run_id: `t_live_${Date.now() % 1000}`, cost_micro_usd: 1200,
      created_at: Math.floor(Date.now() / 1000),
    };
    setFeed(f => [...f, toolEv]);
    await new Promise(r => setTimeout(r, 500));
    const explainEv = {
      id: `ec_ex_${Date.now()}`, session_id: 's_copilot',
      actor_kind: 'agent', actor_ref: 'copilot',
      action: 'experimental:explain',
      subject_kind: 'event', subject_id: toolEv.id, outcome: 'success',
      payload: { text_md: `（${action.label} 已执行——实装时此处由 ${action.tool} 的真 result_data 驱动回答。）` },
      caused_by_event_id: toolEv.id,
      task_run_id: `t_live_${(Date.now() + 1) % 1000}`, cost_micro_usd: 2800,
      created_at: Math.floor(Date.now() / 1000),
    };
    setFeed(f => [...f, explainEv]);
    setTyping(false);
  };
    ...
    <aside className="drawer">
      ...
      <div className="drawer-feed" ref={feedRef}>
        ...
        {feed.map((ev) => {
          ...
          const isToolUse = ev.action === 'experimental:tool_use';
          const isAccept  = ev.action === 'experimental:accept_suggestion';
          const suggested = ev.payload?.suggested_actions || [];

          // §1.6 #3b — accept_suggestion renders as small chip-tag row, not a chat bubble
          if (isAccept) {
            return (
              <div className="msg-accept" key={ev.id}>
                <ActorBadge actorKind="user" compact />
                <Icon name="check" size={11} />
                <span className="msg-accept-label">采纳建议</span>
                <code className="msg-accept-tool">{ev.payload?.tool}</code>
                <span className="msg-accept-chip">{ev.payload?.label}</span>
              </div>
            );
          }

          // tool-use events render as a standalone card, not chat bubble
          if (isToolUse) {
            return (
              <div className="msg agent msg-tool" key={ev.id}>
                <ToolUseCard event={ev} detail={toolUseDetail} />
              </div>
            );
          }
                ...
                <div className="msg-proposal">
                  ...
                  <div className="actions">
                    ...
                    <Button variant="quiet" size="sm" iconRight="arrowR"
                      onClick={() => { onClose && onClose(); nav && nav('mistakes'); }}
                      title="跳到 /mistakes 看完整推理链——drawer 不嵌套 details">
                      查看推理
                    </Button>
                  </div>
                </div>
                ...
                <>
                  ...
                  {suggested.length > 0 && (
                    <div className="msg-actions">
                      {suggested.map((a, i) => (
                        <button key={i} className="msg-action-chip"
                          onClick={() => {
                            if (a.nav) { onClose && onClose(); nav && nav(a.nav); return; }
                            if (a.tool) { acceptSuggestion(a); return; }
                          }}>
                          {a.tool && <Icon name="zap" size={11} />}
                          {a.nav && <Icon name="arrowR" size={11} />}
                          {a.label}
                        </button>
                      ))}
                    </div>
