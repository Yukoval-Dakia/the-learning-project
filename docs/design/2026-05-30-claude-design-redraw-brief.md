...
- **OC-5 "AI 自动录入" review surface** (T-OC, YUK-164 B2 — **backend ready, UI deferred to this redraw**). A new `/record` MODE_TAB (`auto_enrolled`) listing what the WorkflowJudge auto-enrolled / would-enroll, with per-item revert. Reads `GET /api/ingestion/[id]/events` filtered to `action='experimental:auto_enroll_observed'` (observe trail, written even with the enroll flag OFF — useful immediately as a tagging-quality inspector) + the enroll-path marker once the flag is ON. Mutates `POST /api/ingestion/[id]/revert` (block_id → reverts the auto-enrollment: retract event + archived record + block back to `draft`; backend shipped in B1b). Component spec (pre-flight approved): a sibling panel `AutoEnrolledPanel` mirroring how `vision_paper` renders `VisionTab`; list rows show route/confidence/suggested-knowledge + a revert action. Minimal-functional only — the visual is this redraw's job.
- **Review-UI AI prefill** (T-OC, YUK-164 B3 — backend ready, UI deferred to this redraw). For `review`-routed captured blocks, surface the TaggingTask suggestions + MistakeEnroll draft (outcome/difficulty/cause) as editable prefills in VisionTab's review step. Source = the `auto_enroll_observed` event payload (`suggested_knowledge_ids`, `mistake_draft`); event-marker-only (no new `question_block.ai_*` columns per ADR-0026 §4).
