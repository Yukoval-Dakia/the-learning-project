...
5. **三个红线 caveat**：(a) C=4 是**未经数据校准的保守初值**（n=1：不从数据拟合）；(b) 截权**注入偏置换方差**（Ionides 2008 / Bottou 2013 截断权衡）——且偏置方向是**拉向高-π（高频被选）标签的自归一化均值**，即**部分重引入 IPW 本要去除的 selection bias**，**不是**「拉向锚」（λ=1 时锚代数上被消掉）；default-ON 的论证只立足于「首写不可逆 ⇒ 有界方差 > 无偏」，不立足于「保守拉锚」；(c) obp 把 clip 默认 off（∞）——我们默认 ON，因为场景是 small-n firm-up 不可逆资产，非 large-n 无偏 OPE；回滚旋钮 = `C = Infinity`（数学上精确还原**估计器**旧行为，无需删代码）。**Carve-out（独立 review 环 C1/C4）**：`C=∞` 回滚的是**截权**（estimator 输出逐位还原，division-form 保真）；它**不**回滚 `estimateLambdaStar` 的**常数锚短路**——那是独立 bug 修复（旧的 `!(varM>0)` 守卫在加权矩之后、可被 FP 求和噪声逃逸 ⇒ 对非-2-幂锚吐垃圾 λ*），短路后真正先于矩计算，与 cap 正交，恒生效。
...
Schema（`schema.ts:1664-1689`）记 per-item 结果：`π_i`、`policy`、`selected`、`signals` 快照。**不记**：sampler rng seed（prod 曾走裸 `Math.random`）、temperature、完整 pre-sample 权重/q 向量、targetCount、**哪条 fallback 路径生效**、clip 激活。选题决策**部分可重构**（π + signals），**不可确定性回放**（无 seed、无路径标签）。**PR-2 缓解（log-only，非 schema）**：`buildSeededSelectionRng` 用 seeded `mulberry32` 取代裸 `Math.random`，seed **惰性自记**（`console.log('[selection] seeded', …)` 恰在**首次真实抽签**时打，未消费的 rng 零 decoy 日志）。回放 = 看日志 seed + 手喂重跑；seed 列仍 defer（Q-d）。
...
| **C. Median-相对 cap** `[LIGHT, 终裁推荐]` | `w_j ≤ C · median(w)`，median 取该题标签批的未截权重中位数，C 无量纲保守常数。 | ✅ **对 outlier 鲁棒**（median 不被 fluke 抬高——B 的死穴）；✅ **对 N 尺度不变**（median 自动跟踪 honest 权重尺度 ≈ N/n——A 的死穴）；✅ **杠杆闭式可陈述**：honest 权重近同质时单条 capped fluke 质量 ≈ `C/(C+n−1)`，C=4, n=12 → 4/15 ≈ **26.7%**（uncapped ≈99%）⇒ 首写最坏位移 ≈ 0.27×标签跨度（典型 ~0.5 logit，棘轮极端 ~1 logit），C=3 → 21%。⚠️ median over ≤12 点偏粗 + C 仍是手拍——但这两点弱于 A/B 各自的定量失效（draft 原拒绝理由被终裁推翻）。⚠️ 残余风险（连续侵蚀，非「过半才失效」的阶跃）：k 条并发低-π 探索标签的聚合 capped 质量 ≈ `kC/(kC+n−k)`，**自 k=2 起连续侵蚀**（C=4,n=12：k=2 → 44%，k=5 → 74%）；`k≥n/2` 时 median 本身被 fluke 权重抬跳 ⇒ cap 彻底失效。多条并发是**指数级罕见**（每条以概率 π 到达），且 clip 可观测性（G2，clipActivations）使这类批可见——不是无声失效。 |
...
3. `estimateLambdaStar` 同步 cap 但**当前对输出是死路**（常数 m̂ → return 1）。**修后事实（C1）**：`return 1` 现由**显式常数锚短路**（`labeled.every(s => s.prediction === labeled[0].prediction)`）保证，**真正先于**加权矩计算——不再依赖旧的 `!(varM>0)` 事后守卫（那在矩算完之后、且可被 FP 求和噪声逃逸 ⇒ 对非-2-幂锚吐垃圾 λ*，独立 bug 已修）。`!(varM>0)` 保留作 Phase 7+ 非常锚的后备守卫。Phase 7+ 激活后 capped-λ* 是**启发式**（Angelopoulos 闭式在 uncapped 估计器下推导；PPI++ 本身 **preprint-only**，arXiv:2311.01453，同行评审未确认——scout ③ 权威分级照录），非证明过的方差最优。
4. **回滚旋钮 = `C = Number.POSITIVE_INFINITY`**：cap 永不 bind ⇒ 数学上精确还原**估计器**旧行为（未截项走 division-form 单舍入，与 pre-YUK-558 逐位一致；positivity throw 保证 π>0），无需删代码——恰是 obp 的 default-∞ 语义（Lens B-F8）。**λ* 常锚短路例外**：它是独立 bug 修复（C1），**不随 `C=∞` 回滚**（旧代码对非-2-幂锚吐 FP 垃圾 λ*，短路后先于矩计算恒生效）。
...
- **`estimateLambdaStar`**（`:202-215`）：同 cap 进 `wSum/wMean/cov/varM`。**修后（C1）**：先跑**显式常数锚短路**（`labeled.every(prediction === labeled[0].prediction) → return 1`），**真正先于** cappedIpwWeights 与加权矩——不再靠事后 `!(varM>0)`（那可被 FP 噪声逃逸，对非-2-幂锚吐垃圾 λ*）。`!(varM>0)` 保留作 Phase 7+ 非常锚后备守卫。**call-site 现场注释**（Lens B-F9，per 项目 phase-deferred-comment 惯例）：「常锚短路 → cap 当前对输出 no-op；Phase 7+ 非常数 m̂（`:456-460`）激活后为启发式（λ* 闭式在 uncapped 估计器下推导），非证明过的方差最优」。
...
- `import { mulberry32 } from '@/server/calibration/rng'`（**不新写 PRNG**）。seed = 简单整数 hash(localDate ‖ 事件种别 ‖ 触发 id)，**惰性记录进日志**（C2：日志推迟到 rng 首次真实被调用、恰打一次 → 已物化流 GET / no-op nightly / 非 rerank PATCH 零 decoy）。
- 注入点（Lens B-F6 修正后的 touch 面）：
  - compose/物化事件：prod caller 构造 `deps.rng = buildSeededSelectionRng(date, 'compose'|'compose-nightly'|'recompose', triggerId)` 传入 `softmax-selection.ts` 的 compose deps（消费点 `:376`）。**triggerId 语义分野（C9+C10③）**：`compose`/`compose-nightly` 用 `date`（**物化幂等是真 feature**——同日重跑 = 同选集，双重检查命中不 double-compose）；`recompose` 用 `newId()` **nonce**（手动重排无自然稳定触发 id，nonce 保留「每按新抽」语义，replay 凭日志记录的 seed）。
  - 重排事件：`advanceStreamItem`（`stream-store.ts:708`）的 prod caller（route handler / job）注入 `rerankDeps.rng`（→ `:743` → `:899` → `:1076`），**独立 seed**（triggerId=被推进的 itemId；localDate 只做 seed 命名空间熵，rerank 实跑用 `updated.date`；itemId 全局唯一 + done 终态无重入 ⇒ 该 itemId 的 rerank 抽签至多一次，seed 稳定可回放）。
...
| **Q1 回滚恒等（estimator 级，C4）** | `C = Infinity` ⇒ **estimator 输出**与 pre-YUK-558 division-form **逐位相等**（内联旧公式复刻，随机 π/label/λ 批 `toBe`；未截项走 `xi/π` 单舍入保真）。**λ* 常锚短路除外**（C1 独立 bug 修复，不随 `C=∞` 回滚）。 | unit |
| **Q1 λ*==1 inert-in-phase + 常锚短路（C1）** | 常数锚 → `estimateLambdaStar` capped 后仍返回 1；anchor ∈ {0.1, 0.7, −0.3} × C ∈ {4, ∞}（11 honest + 1 fluke fixture）全断 `λ* === 1`（显式短路先于加权矩）。 | unit |
| **C6 k≥2 并发侵蚀（诚实失效模式）** | k=2（clipped=2，聚合质量 ≈ 44.4% = `kC/(kC+n−k)`）+ k=6≥n/2（clipped=0，median 抬跳 → cap 彻底失效钉死）。 | unit |
| **C7 positivity 有限性** | 每 estimator（aipwMean / ppiPlusMean / estimateLambdaStar）+ 一条 `pi = Infinity` throw（`Number.isFinite` 收紧）。 | unit |
| **C8② 偶数异质 median** | `pis=[0.5,0.25,0.2,0.02]` → raw`[2,4,5,50]`，median=`quantile(0.5)`=4.5、cap=18 → `weights[3]===18, clipped===1`（误用 `sorted[mid]`=5 会给 20，被击杀）。 | unit |
| **Q2c clip 可观测** | fluke 批 → `RecalibrateResult.clipActivations === 1`，`minPi === 4e-4`；nightly 聚合**累加**（Q1=1-fluke + Q2=2-fluke → `clip_activations === 3`，非单赋值 1/2）。 | unit + db |
...
| **Q6 seed 确定性 + 惰性日志（C2）** | 同 seed → `sampleByWeight` 选集逐位相同；未消费 rng → 零日志；首消费恰一次 log；数值流逐位 = 裸 `mulberry32(seed)`。 | unit |
| **C8① route 级 e2e determinism** | GET compose → 快照选集 → 删物化流(+观测行) → 再 GET → 选集逐位相同（seeded 抽样可重构）。 | db（`stream.db.test.ts`） |
| **C5+C8④ 追溯 recompute** | 2 firm-up 题(1 fluke) → considered=2/recalibrated=2/clip_activations≥1/min_pi_seen=4e-4；重跑 → b_calib 逐位不变（值幂等）；π=0 毒标签题 → skipped_failed=1 不阻断。 | db（`tests/integration/recompute-bcalib-cap.db.test.ts`） |
...
- **Q-g（追溯 recompute——Q2d，新增）**：PR-1 是否附带对存量 `b_calib IS NOT NULL` 题的一次性 recompute pass（**值幂等**——同标签集 ⇒ 同 b_calib 逐位不变；但**时间戳写非幂等**——`last_calibrated_at`/`updated_at` 每跑无条件刷新 `new Date()`，故重跑非零副作用；有界；当前存量可能为零 ⇒ no-op）？（Designer 默认：**yes**，成本近零。）
...
| **Evidence-first 可追溯可回滚** | 不可逆首写配**同窗口** firm-up audit（PR-1，clip/min-max π/λ*）；选题决策 seed 化可重构（PR-2，惰性日志——首次真实抽签才记，未消费零 decoy）；回滚旋钮显式（`C=∞` 精确还原 estimator，division-form 保真；λ* 常锚短路 C1 例外，不随回滚）；追溯 recompute 通道（Q-g，值幂等 + 时间戳写非幂等）。 |
...
| B-F8 | 回滚旋钮未陈述 | **ACCEPT（LOW，已修入 Q1 rider 4 / M1 docblock）** | 适配 C 形后回滚值 = `C = Number.POSITIVE_INFINITY`（cap 永不 bind ⇒ estimator 逐位还原，未截项 division-form 单舍入；positivity throw 保证 π>0）——恰为 obp default-∞ 语义；§6 加 estimator 级回滚恒等测试。**λ* 常锚短路（C1）不随回滚**——独立 bug 修复，与截权正交。 |
...
### 独立 review 环裁决（2026-07-03，fable 终裁 + executor 落地）

reconcile 终稿 ship 后另起一轮独立对抗 review（10 簇候选 C1-C10），fable 逐一自验拍板；executor 照单实施。裁决分类：

**修入（landed，commit `fix(practice): YUK-558 — 独立 review 修复`）：**

| 簇 | 修法 | 落点 |
|---|---|---|
| **C1** | `estimateLambdaStar` 常数锚**显式短路**（`labeled.every(prediction === labeled[0].prediction) → 1`）真正先于加权矩——旧 `!(varM>0)` 事后守卫可被 FP 求和噪声逃逸（对非-2-幂锚吐垃圾 λ*）。**独立 bug 修复，不随 `C=∞` 回滚**；`!(varM>0)` 保留作 Phase 7+ 后备守卫。 | `recalibration.ts` estimateLambdaStar |
| **C4** | `aipwMean`/`ppiPlusMean` 未截项走 `x/π` **单舍入**除法形（`w === 1/π` 精确判别）——与 pre-YUK-558 逐位一致；C=∞ 全走此分支 ⇒ estimator 回滚恒等升为 division-form 逐位 `toBe`。 | `recalibration.ts` 两 estimator 循环 |
| **C3** | `recalibrateQuestion` 顶部 hoist `const capFactor = IPW_WEIGHT_CAP_C`，显式传 estimateLambdaStar/ppiPlusMean/cappedIpwWeights——观测与 estimator 同源。 | `recalibration.ts` recalibrateQuestion |
| **C7** | 三 estimator positivity guard 收紧 `!(Number.isFinite(π) && π > 0)`（消息含「finite number > 0」），各加 `π = Infinity` throw 测试。**不**在 cappedIpwWeights 加第四道 assert。 | `recalibration.ts` × 3 + 测试 |
| **C10④①②** | `medianOfWeights` → `quantile(raw, 0.5)`（删本地 helper，复用 `@/core/theta`）；删 `recalibration_nightly.ts` dead import `isNotNull`；recompute 脚本相对 import → `@/server/mastery/recalibration`。 | 三处 |
| **C2** | seed **惰性自记日志 thunk**——首次真实抽签才 log、恰一次，未消费零 decoy。 | `selection-seed.ts` + 新 `.unit.test.ts` |
| **C9+C10③** | `recompose` triggerId → `newId()` nonce（无自然稳定触发 id）；compose/compose-nightly 保 date（物化幂等真 feature）；PATCH 处注释 date=命名空间熵。 | `stream.ts` |
| **C5+C8④** | recompute docblock 拆「值幂等（同标签集⇒同 b_calib）」vs「写非幂等（时间戳每跑刷新）」；新增 recompute db 测。 | `recompute-bcalib-cap.ts` + 新 db 测 |
| **C6** | `IPW_WEIGHT_CAP_C` docblock + spec Q1-C 补 k≥2 连续侵蚀闭式（`kC/(kC+n−k)`；k≥n/2 median 跳变 cap 失效）；unit 测 k=2/k=6。 | `recalibration.ts` docblock + spec + 测试 |
| **C8②③⑤** | ② 偶数异质 median 测（`sorted[mid]` 回归击杀）；③ nightly 聚合 Q2=2-fluke → `clip_activations === 3`（累加非单赋值）；⑤ flukeMass 重言断言替换为从 estimator 输出反导（`impliedMass = bCalib/2.0`）。 | 三处测试 |
| **C8①** | route 级 e2e determinism（`stream.db.test.ts`：GET compose → 删物化流 → 再 GET → 选集逐位相同）。 | `stream.db.test.ts` |

**Defer（本轮不做）：** C3 estimator 返回形状重构（多消费者时重议）；C10⑤ fold 去重（WONTFIX-nit）；C10③ 结构性 seed 下沉（注释足够）。

**REFUTE / 修正：** C1 finder 方向转置（实质裁决不变，病根在旧代码，非本 PR 引入）；C8① DI 多跳「零覆盖」不实（`stream-store.db.test` 既有覆盖，缺口收窄至 route 3 行 seed 接线——已由 C8① route 测覆盖）。

**CLEAN 复核维持：** 命门 1/2/5；FNV-1a / mulberry32 复用（非新写 PRNG）；conventions 零违反。
...
2. **Q-b cap 数值**:owner 拍 **C=4**(单样本杠杆 ≈27% @n_min=12,域 3-5 中位)。代码注释原样标注「未经数据校准的保守初值,非拟合结果」;回滚 = `C=Infinity` 逐位还原 estimator(division-form 保真;λ* 常锚短路 C1 例外,不随回滚)。
