...
> **⚠️ 2026-06-15 amend（difficulty-data-driven-research 升级 G2 严重度）**：G2 从「中文 K12 各科零直证」**升级为「有直证且偏负面」**——`difficulty-data-driven-research`（`docs/design/2026-06-15-difficulty-data-driven-research.md` §D）核到 **ZPD-SCA（arXiv:2508.14377, 2025）**：Qwen-max / GLM zero-shot 评中文 K12 认知难度**低于随机猜测**，且这两个模型正是本项目栈近亲。即「中文域 × 开放/主观题型」交叉格不是「没证据」而是「有负面证据」，且 PPI 无法兜底（开放题真值非客观闭环）。该格的 feature-based b 锚必须标 **propose-only + 显式低置信**，靠 owner 复盘事后校验。详见研究文档 §4 诚实天花板。
