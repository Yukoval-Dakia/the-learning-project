...
> **⚠️ 2026-06-15 amend（difficulty-data-driven-research §E 升级）**：上句「落地细节」**升级为明确指定**：阶段③ PPI 纠 b 必须用 **active-PPI（Zrnic & Candès, ICML 2024）的 IPW 加权 rectifier**（每项 ÷ 选中概率 π_i），基础 PPI 的均值 rectifier 在 active-learning 非 i.i.d. 锚题下**有偏**、会把锚标尺校歪。落地用 **PPI++ power-tuning λ\***（锚质量自适配，λ\*→0 烂先验自动归零）。**实施硬缺口**：active-PPI 必需**持久化每锚题的选中概率 π_i**——当前选题引擎（ADR-0042）设计未存 π_i，这是 deferred recalibration #45 的设计输入第 1 项。另：PPI 纠的是**分布/标尺层系统偏差**（非逐题 b），逐题 b 单点 n=1 仍只能 fixed-anchor 反推（CI 宽）。详见研究文档 §1-§2。
...
> **⚠️ 2026-06-15 amend（difficulty-data-driven-research 升级 G2 严重度，范围限中文阅读/语文）**：G2 对**中文阅读理解/语文**这一格从「零直证」**升级为「有直证且偏负面」**——`difficulty-data-driven-research`（`docs/design/2026-06-15-difficulty-data-driven-research.md` §D）核到 **ZPD-SCA（arXiv:2508.14377, 2025）**：Qwen-max / GLM zero-shot 评**中文阅读理解**认知难度**低于随机猜测**，这两个模型正是本项目栈近亲。**适用范围（WebFetch 核实摘要 + Codex review）：ZPD-SCA 只覆盖中文阅读理解，不含数学——中文数学/理科/客观题对它仍是「缺直证、未验证」，不按已反证处理。** 即「中文阅读/语文 × 开放/主观题型」交叉格不是「没证据」而是「有负面证据」，PPI 无法兜底（开放题真值非客观闭环）；该格 feature-based b 锚必须标 **propose-only + 显式低置信**，靠 owner 复盘。详见研究文档 §4。
...
> **⚠️ 2026-06-15 amend（difficulty-data-driven-research §1/§4）**：阶段③启动后，锚标尺**可被 owner 客观题经 active-PPI 持续 rectify**——硬轨地基不再纯软（系统性平移/标尺偏差可数据驱动纠正）。**但仍受两条硬墙约束**：(1) b 永远不能脱离外部锚成为 owner 自证的量（最强止于「半数据驱动」：数据去偏 + 锚定尺度，非「全数据驱动」从零定 b）；(2) PPI 纠分布/标尺层，**逐题 b_j 单点 n=1 仍无法精确定**。所以「锚来源 + 锚质量」仍是独立风险项，但严重度从「站在软地面」降为「软地面 + 可持续校准的标尺」。
...
- **ML 难度预测当 Bayesian IRT prior**：Bock & Aitkin 1981 的 MMLE/EM 边际似然框架不支撑外部 ML 预测直接充当 Bayesian item prior。prior 形式、校准方法与证据来源须另行选定并验证；补齐前，具体 ML 输出只作候选输入。
...
**核验诚实声明**：审计对 22 个不重复来源做了独立联网核验（含全部 arXiv ID + 最 load-bearing 的数字），**裁定无任何一条疑似编造**。本表已落实审计全部修正：H1（r≈0.87 ID 改 2504.08804）、H2（SMART 升级为 EMNLP 2025 CORE-A）、M1（直接 prompt r≈0 承重源 = Acquaye，勿混 Razavi direct）、M2（PFA>BKT 标机制转述非实证）、M3（b=β=D 降格 needs linking）、L1（Corbett 统一 1995）、L2（LFA 统一 2006）、L3（Hoyl 方法 NN+IRT 非 random forest）。#14 Dueñas 不可引用、未精确溯源的 ML→IRT 工作已删除署名、#23 RUM 部分二手——均不作净结论承重。最 load-bearing 的 Stocking 1990 a/c 死路句、Gervet 2020 PFA 占优句、Klinkenberg 2011 Math Garden 数字、PPI Science 五作者均逐字核到。
...
**owner 需在 B1 文档落实的修补点**：(1) `b=β=D` 等号 → 「同 logit 语义，需 linking 对齐」（§6.1）；(2) ML 难度预测接入 IRT 的 prior 形式、校准方法与可核验来源**尚未落实**，绑定既有 **YUK-439**，该项完成前不得把 ML 输出当成已验证 Bayesian prior（§7.5）；(3) r≈0.87 ID 改 arXiv:2504.08804、r≈0.78 方法改 NN+IRT、SMART 标 EMNLP 2025（§7.3 H1/H2/L3）；(4) Urnings 作者改 Bolsinova et al. 2022（§5.3 L6→实为 Urnings 作者修正）；(5) 慢热第④阶段降级为「propose-only + 埋点验证的产品假设」（§7.2 G1）；(6) `item_calibration` 软轨列 allowlist 须标「置信上限受 n=1 无 cohort 约束，非纯时间问题」（§6.3）。

**Linear**：本任务为四路调研 + 权威审计的 cross-统合存盘，产出即本地基文档正文，无代码/schema 改动。ML→Bayesian prior 的未决验证由既有 **YUK-439** 追踪；其余 actionable 修正项归属既有 B1 / YUK-203 P3 / YUK-344 范畴。无需为同一 prior gap 再建重复 issue；若 owner 要独立追踪两个外推缺口（G1 开放/主观题型外推无文献、G2 本项目内容域 LLM 标定零直证），建议作为 comment 追加到 B1 对应 issue（YUK-203 P3 或 calibration 子 issue），不碎片化新建。
