...
- **GPT「四诊断器」(IRT/CDM/KT/LLM)的最终去向 —— 全诊断栈照算照留(分轨标置信),但只有一个 PFA 作可信决策信号(+LLM 先验)**。否决的是 GPT「四个并行引擎都进决策」的 router,**不是**否决计算/保留它们的数据(理由 = n=1 稀疏数据养不起四个**可信**引擎,且四者大量重叠):
  ...
  - ⚠️ **来源诚实**:此条是综合连接 [difficulty=PFA β=FSRS D(已落地)] + [Phase 1 §5 选 PFA 弃四引擎] + 标准心理测量 + n=1 限制;四诊断器深挖原文未存盘,系重建非逐字引用。ML 难度预测接入 IRT 的正式依据仅采用已核验的通用贝叶斯 IRT + 先验原理（Bock & Aitkin 1981 的 MMLE/EM 边际似然框架），具体 ML 输出只作需实证校准的先验输入。
