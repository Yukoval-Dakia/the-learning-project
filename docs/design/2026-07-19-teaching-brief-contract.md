...
export interface TeachingBriefEvidenceRef {
  role: 'induction' | 'probe' | 'outcome';
  kind: string;
  id: string;
}
...
export interface TeachingBriefPlanImpact {
  summary_md: string;
  rationale_md: string;
  evidence_trace: TeachingBriefEvidenceRef[];
}

export interface TeachingBriefMethodChoice {
  method_id: string;
  summary_md: string;
  rationale_md: string;
  evidence_trace: TeachingBriefEvidenceRef[];
...
export interface TeachingBriefActionExtensions {
  /** YUK-505 seam；P0F 必须省略，不能传 null 或占位文案。 */
  plan_impact?: TeachingBriefPlanImpact;
  /** YUK-506 seam；P0F 必须省略，不能传 null 或占位文案。 */
  method_choice?: TeachingBriefMethodChoice;
}

export type OutcomeConfirmedPreparedAction = TeachingBriefActionExtensions &
  (
    | { kind: 'none' }
    | { kind: 'acknowledge_outcome'; probe_result_event_id: string }
    | { kind: 'practice_kc'; knowledge_id: string }
  );

export type OutcomeRetiredPreparedAction = TeachingBriefActionExtensions &
  (
    | { kind: 'none' }
    | { kind: 'acknowledge_outcome'; probe_result_event_id: string }
    | { kind: 'continue_today' }
...
export interface FindingTeachingBrief extends TeachingBriefBase {
  state: 'finding';
  expires_at: string;
  prepared_action: TeachingBriefActionExtensions & {
    kind: 'review_finding';
    proposal_id: string;
...
export interface OutcomeConfirmedTeachingBrief extends TeachingBriefBase {
  ...
  expires_at: string;
  prepared_action: OutcomeConfirmedPreparedAction;
  current_outcome: {
    status: 'confirmed';
    summary_md: string;
    probe_question_id: string;
...
export interface OutcomeRetiredTeachingBrief extends TeachingBriefBase {
  ...
  expires_at: string;
  prepared_action: OutcomeRetiredPreparedAction;
  current_outcome: {
    status: 'retired';
    summary_md: string;
    probe_question_id: string;
...
- 四块内容在四个分支都必填；不得用 `undefined`/`null` 省略某块。
...
- 完整链必须同时满足：question `source_ref` 与 `metadata.conjecture_proposal_id` 都等于 proposal
  id；result 的 `subject_id` 等于 question id；result `payload.conjecture_event_id` 与
  `caused_by_event_id` 都等于 proposal id。任一关联不一致都按 corrupt candidate 跳过。
...
- 唯一允许的 `null` 是 `probe_ready.expires_at`。YUK-505/506 尚未产出时，
  `plan_impact` / `method_choice` 键必须**缺席**，不得传空对象、`null` 或假内容。
...
- P0F/2 尚无 outcome ack 或 remediation SoT，所以 outcome 的 `prepared_action` 必须是
  `{kind:'none'}`。P0F/4 落地 ack 后才可返回 `acknowledge_outcome`；YUK-709 接上现有
  KC-scoped practice 后，confirmed 才可返回 `practice_kc`，retired 才可返回
  `continue_today`。不得提前返回尚不可执行的 action。
...
| orphan mind-probe question | 缺 proposal、双 provenance 不一致或 proposal 非 accepted 时跳过并记录 | 不展示无依据题目 |
| orphan/corrupt result | 缺 question/proposal，或 `resolution/outcome` 不合法时跳过并记录 | 不展示伪结论 |
...
规划脑和教学法脑只能进入 `prepared_action.plan_impact?` 与
`prepared_action.method_choice?`，不增加第五/第六个顶层内容块：
