...
| # | 决策 | 依据 | 状态 |
|---|------|------|------|
| 1 | **七条产品红线**：单用户无多租户 / AI 可追溯可回滚 / 事件是唯一真实来源(correction 唯一撤回) / 科目是视角不是结构(派生轴禁 subject 列) / 原图必须同步留存 / 树是骨架网是肌肉 / 写入仅 propose | spec §6 + D13 + ADR-0006v2/0007/0010/0012 | locked（③ 实现状态见 ADR-0044：event-sourcing 改造 Wave 0-3 进行中、θ̂/FSRS 走快照语义；② 可回滚以其〈诚实天花板〉为界；⑦「写入仅 propose」= spec §6 + D13 双层能动性——AI 地盘/草稿层当场改、用户数据 propose） |
...
| 7 | **破坏性/用户数据 mutation 无直接 write tool，全 propose-only**；AI 地盘/草稿层直写须携 owner ADR 编号（现行 ADR-0031/0032/0033），例外清单住 `allowlists.ts` 注释、不烤进条文（2026-07-07 A1 审查：向同文档准确作用域句 §「破坏性动作无直接 write tool，全 propose-only」对齐，消除与依据列的字面矛盾） | registry.ts + allowlists.ts | locked |
