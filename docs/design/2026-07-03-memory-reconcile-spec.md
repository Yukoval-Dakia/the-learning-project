...
3. **捕获什么**：
   - `prev_text`：SUPERSEDE/MERGE **优先** `capturePrevState` 读到的 DB 快照原文（`snap.text`，verbatim
     保真），快照缺失（防御式：行已不在）时兜底 `oldMem.text`（`CandidateEntry.text`，作用域内）——PR #699
     CR-5：search candidate text 可能与 pgvector payload 漂移，故 undo 以 DB 快照为主、candidate 为 fallback；
