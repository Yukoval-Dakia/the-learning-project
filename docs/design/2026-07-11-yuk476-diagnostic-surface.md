...
- **状态**：DRAFT — 等 owner 判词（设计 lane 停在草稿，不越界进实施）
...
**做**：把已上线的 profile 数据在 `/today` 冷启态露出一张诚实的「起始画像」band 卡片；把 `/profile` 深读面从孤儿变持久可达；把冷启/薄证据/未测各态设计成一等诚实空态。**零后端改动。**
...
- **`/today` 卡片（新建）**：一个 block，`LoomCard + SectionLabel + 连续 band 原语` 组合，挂在 `TodayPage` 的 `KpiRow`（`:393-399`）与 `今日之线`（`:401`）之间，gate 在 `active` goal——**完全复刻 `OvernightDigestBand` 模式**（`TodayPage.tsx:234-311`）。卡片读 `active.goalId`（已在 `TodayPage` scope 内）→ `getPlacementProfile(goalId)`。是**摘要条**（testedCount/totalKcs + 最弱 2–3 个 KC 迷你带 + CTA），非 `/profile` 的复制。
...
**建（v1）**：1 个 `ProfileBand.tsx` block + 复用 `.ob-band` + 挂 `TodayPage`（gate active goal）+ 5 态（A–E）+ 连续带（宽=不确定）+ `p_l` 标记 + 软/硬 pill + low-confidence 饱和 + 截断页脚。零后端改动。
...
1. **`/profile` 持久机制**：卡片 CTA-only（gate 掉即重孤儿）vs 轻量持久入口（知识/练习面）vs 顶层 NAV tab。取舍：可发现「区间随时间收窄」的价值 vs 单用户工具的导航杂乱。**推荐：轻量持久入口，非顶层 tab。**
2. **多目标**：端点严格 per-goalId。多目标时 `/today` 卡片只显 active goal，还是聚合？取舍：零后端（active-only）vs 需新后端且无 live 消费的 roll-up。**推荐：v1 仅 active goal。**
3. **来源 pill（软轨/硬轨）无后端字段**——两席都派生，但规则不同（保真席 `硬轨 iff tested && evidence≥3 && !low_confidence`；真相席 `软轨 iff low_confidence || evidence<3`）。v1 定一条，还是请后端加 per-KC `calibrated`/`source` 字段？**推荐：钉真相席规则做 v1；provenance 成载重再请后端字段。**
4. **4 档 label 作次级读**：仅宽度（推荐）vs 连续带上方叠一个粗档 label（真相席 D6 允许）。取舍：熟悉的粗把手 vs 重引入本面要避免的假精度。**推荐：冷启仅宽度。**
5. **深读面裸点 %**：静息仅区间 %（= 线上 `ScreenProfile.tsx:331`），点作标记；裸点 `p_l`+`SE` 放 hover/expand（真相席 §5）还是完全不露？取舍：可检视 vs 一个孤零「61%」读成假点估计。**推荐：点+SE 仅 hover；静息 = 区间+标记。**（这也消解了「露不露数字」的原型内部不一致——线上本就印区间、不印裸点。）
6. **#41 校验 badge 在深读面**：保留可见（当前启用、诚实 `'libm'`「预览·待 σ 对齐」）vs 藏到 YUK-508 翻转后。取舍：现在就强化 evidence-first/可复现原则 vs 一个 YUK-508 前说不了「逐位校验 ✓」的「预览」标。**推荐：保留可见——诚实且已上线；文档不得暗示 YUK-508 前 bit-exact。**
7. **`evidenceCount` 文案**：确认设计稿从不把聚合渲成「N 题」（§9-2）。
...
*本文件为 DRAFT，等 owner 判词后再定稿并转实施。设计 lane 不越界进实施。*
