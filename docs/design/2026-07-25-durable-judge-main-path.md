...
| **Exhausted / permanent failure** | each failed attempt's `catch` wrote a `judge_run.attempt_failed` trace (§3.1); on the final attempt the job lands in `judge_run_dlq` (dead-letter, per `jobQueueOpts`) with **no further handler invocation** — the last `attempt_failed` trace is the terminal record. | "本次判分暂不可用 — 已记录，稍后可补判" — explicit degraded-final, NOT a silent wrong-answer. Attempt remains recorded but **unjudged** (never scored as wrong; that was the whole point of the F4 photo-only gate lineage). | Manual **unjudged-run re-enqueue** (§3.6b domain-scan mechanism, triggered for a specific `run_id`). **NOT `rejudge.ts`** — that job is an appeal flow (`appeal_event_id` input, requires an existing `appeal_request` + a prior judge verdict; an unjudged DLQ run has neither → it would be skipped). DLQ auto-sweep NOT built (RULED D6, manual-only). |
...
- **(b) Crash-window between dispatch steps → reconcile off the DOMAIN state, NOT off `job_events`.** Dispatch does several non-atomic things — write the pending/unjudged attempt/outcome **domain event** (the invariant that is written first and reliably, `judge: pending`), write the `QUEUED` `job_event` marker, and `boss.send` — which are not one tx. **Any recovery keyed on `job_events` existence is broken** (grounded codex correction): a crash *after* the domain event commits but *before* the QUEUED marker / `send` leaves a run with **no `job_events` row at all** — invisible to a `job_events.queued` scan AND to the `/status`-by-run-id path. Conversely, if #1068 uses **send-first** ordering, the window is "pg-boss job exists but the QUEUED marker was never written" — again a `job_events` scan misses it. **So the recovery source of truth must be the domain state itself.** **Remediation:** a periodic **reconcile sweeper scans the DOMAIN event log** for attempt/outcome events in a *judge-pending / unjudged* state (no terminal verdict written for them — no `judge` event, no worker backfill) older than a stall threshold, and **re-enqueues a `judge_run` for each**, keyed by the pending domain event's own id as `run_id`. This is independent of whether any `job_events` row or pg-boss job exists, so it covers **both** crash-window shapes (marker-first: marker-but-no-job; send-first: job-but-no-marker), and the request/redelivery idempotency keys (c) prevent a double judge when an "invisible" in-flight job actually does run. The repo already has the durable-write pattern to lean on — the **event-write outbox** (`docs/adr/0021-event-write-outbox-pattern.md`, `drizzle/0017_outbox_event_ingest.sql`): the window-closing form enqueues the send-intent in the *same tx* as the pending domain event, a relay drains to pg-boss — but the domain-scan sweeper is the backstop that needs no ordering assumption. **Wave: W2-hardening** — ship the domain-scan sweeper with the submit flip (a stalled unjudged attempt is a real user-visible permanent-pending); the outbox-relay form can follow if the sweeper proves insufficient.
...
| **W2 — submit face to main path** | Switch `submit.ts` to enqueue + 202-pending (auto_rate path only; manual rating stays synchronous, §2.1); UI "判题中 → 回填"; FSRS + auto-rate deferred to backfill (D4); wire the §3.6 hardening: (a) `checkRateLimit` HTTP-side before enqueue, (b) domain-scan reconcile sweeper (scans judge-pending domain events, NOT job_events) + a manual unjudged-run re-enqueue entry, (c) request-idempotency key + dedupe, (d) `/status` domain-event fallback. | Submit returns 202-pending; attempt recorded unjudged; verdict + FSRS land on backfill event; endpoint-down soak (kill the endpoint, confirm cross-provider recovery within `1+retryLimit`); rate limit enforced once at HTTP boundary; a crash between the pending domain event and enqueue is recovered by the domain-scan sweeper; a DLQ-exhausted unjudged run can be re-enqueued manually (not via `rejudge.ts`); a duplicate POST returns the same run (no double inference). |
| **W3 — remaining faces** | probe-answer, solve-session, paper-submit, advice → enqueue. paper rides per-question `judge_run`. Each face's judge-pending domain state (probe `probe_result`-pending, solve/paper attempt-pending) is scannable by the same §3.6b domain-scan sweeper + unjudged-run re-enqueue entry. | Each surface returns pending + backfills; probe `probe_result` / solve mistake-enrollment / paper claim-release all move to worker tx; the domain-scan sweeper + manual re-enqueue cover every face's crash-window and DLQ-exhausted runs; full cross-face coverage-list ticked. |
...
- **D6 — DLQ recovery policy — RULED: manual-only for now; no auto-sweep subsystem yet.** ✔ Don't build auto-recovery before observing real DLQ traffic (避免 建成不通电). **Correction:** the manual recovery entry is a **dedicated unjudged-run re-enqueue** on the §3.6b domain-scan mechanism (re-enqueue a `judge_run` for a specified `run_id` whose attempt is still judge-pending) — **not** the existing `rejudge.ts`, which is an appeal flow keyed on `appeal_event_id` and requires a prior judge verdict, so it structurally skips unjudged DLQ runs. W2/W3 must ship this real entry; do not lean recovery on the semantically-mismatched appeal job.
