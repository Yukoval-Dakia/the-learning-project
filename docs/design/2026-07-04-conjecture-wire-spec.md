...
### Q2 — Probe answer 入口（answerProbe wire）→ **A. conjecture card UI + judge run() pure**

owner 手评不合理（conjecture 的 `baseline_p`/`predicted_p` 是机器算的，对照也得机器判分才公平）。评价走 **judge capability 的 `run()` pure function**（exact/keyword/multimodal_direct/steps/semantic，全 pure 不写 FSRS/θ̂ → **不破 ND-5**）。**不走 `submit.ts` 的 `judgeSubmit`**（那是 attempt 域，触发 θ̂/FSRS update，破 ND-5）。
...
4. 新 route `POST /api/conjecture/probe/:id/answer`（token gate）→ judge run() pure 按 `defaultJudgeKindForQuestion`（`judge-routing.ts:41`）路由 capability → 调 `answerProbe({ outcome, resolution, answer_md })`
...
| 红线 | 守卫机制 | 验证锚点 |
|------|---------|---------|
| **ND-5**（probe 不写 FSRS/attempt/θ̂） | `answerProbe` 结构守（只写 `experimental:probe_result` event）+ judge `run()` pure（不写 FSRS/θ̂）+ 不走 `submit.ts` judgeSubmit | `probe-lifecycle.ts:194` 注释「Writes NOTHING else — no attempt event, no FSRS row (ND-5)」+ `src/core/capability/judges/*.ts` run() 无 FSRS 写 |
...
| 红线 | 修正后守卫 |
|------|-----------|
| ND-5 | judge `run()` 不写 FSRS/attempt/θ̂（exact/keyword/steps/semantic pure；multimodal_direct 调 LLM 产生 cost 但不写学习状态）；不走 `submit.ts` judgeSubmit |
