...
3. **owner 拍板批（全是拍字不是工程）**：YUK-579 三决策点（preflight 已备）、
   redline 菜单批①三项（P5 owner-merged / P1 分层措辞 / P9 路由刷新——J4/J5 给了
   现成裁决框架）、YUK-536 misconception Tier。
4. **redline 菜单批②**：P2 cockpit 滚存归档（⚠️ 保头部日志原文——P5/P9/A5 三份
   KEEP 的证据锚点在其中）+ X6 ADR-0025 改写 + northstar fixture 修复（唯一
   接近真 bug 的项：守卫恒绿）。
...
- **未审尾巴**：redline sweep 溢出 2 条（`kg-mesh-no-tree-expressed-edge` /
  `credit-decay-weight-vs-encompassing-weight-separation`）；portfolio §5 红线
  挑战组 3 条（已有对抗记录在案，只等拍板）。
