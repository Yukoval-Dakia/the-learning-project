...
function QIndicators({ q }) {
  const a = qAssoc(q);
  const subj = QSUBJECT[qSubject(q)];
  return (
...
function ScreenQuestions({ go, ui = {} }) {
  ...
  const reset = () => { setStatus("all"); setSubject("all"); setSource("all"); setKind("all"); setDiffs([]); setLabels([]); setQuery(""); };
  ...
  const filtered = top.filter((q) =>
    ...
    (subject === "all" || qSubject(q) === subject) &&
  ...
  const activeFilters = (status !== "all") + (subject !== "all") + (source !== "all") + (kind !== "all") + (diffs.length ? 1 : 0) + (labels.length ? 1 : 0) + (query.trim() ? 1 : 0);
  ...
  return (
    <div className="page view">
      ...
      <div className="qb-filterbar">
        <div className="qf2"><span className="qf2-l">科目</span>
          <div className="qb-seg">
            {[["all", "全部"], ["yuwen", "语文"], ["math", "数学"], ["eng", "英语"]].map(([s, l]) => (
              <button key={s} className={subject === s ? "on" : ""} onClick={() => setSubject(s)}>{l}</button>
            ))}
          </div>
        </div>
