...
| `knowledge.approval_status` | 🟡 init-only | 期望 admin/AI 流转 pending→approved，**UPDATE path 缺失**；细节见下方附录 `knowledge.approval_status`（YUK-422，reserved-for-future） |
...
## 附录 · `knowledge.approval_status` 的 reserved-for-future 枚举（2026-06-19 · YUK-422）

`knowledge.approval_status` 是三值枚举 `['pending', 'approved', 'rejected']`，但**今天完全 INERT**：
所有行都被自动 approve，且**零读者**。owner 决策是**记录、不删**（document-not-drop）。

| 值          | 语义                                          | 有写路径？ |
| ----------- | --------------------------------------------- | ---------- |
| `'approved'`| 已批准（唯一可达态——所有行都是它）            | **是**     |
| `'pending'` | 待 AI/admin 评审（reserved，未实现）          | **否**     |
| `'rejected'`| 评审驳回（reserved，未实现）                  | **否**     |

**当前实况**：

- **6 个写者全部硬编码 `'approved'`**：`knowledge/proposals.ts:262/355`、`knowledge/seed.ts:40`、
  `orchestrator/learning_intent.ts:526/561`、`scripts/seed-synthetic.ts:238`；schema 列与
  `core/schema/index.ts` 的 `KnowledgeInsert` 也都 `.default('approved')`。
- **零读者**：没有任何查询 filter 这一列，也没有任何行为 gate 在它上面（`'pending'` / `'rejected'`
  literal 在生产代码里出现的几处都属于**别的列**——`knowledge_proposal.status`、
  `question.generation_status`——不是 `knowledge.approval_status`）。
- 因此 `'pending'` / `'rejected'` 是 **unreachable-by-design**：它们为一个**尚未构建**的
  AI-review/approval 流预留——届时 AI 提议的知识会先落 `'pending'` → 被评审 → 转 `'approved'` 或
  `'rejected'`，且读路径会 filter 到 approved-only。

**审计契约**：列级 `pnpm audit:schema` **照常通过**——它只看「该列是否有 write path」，而 `'approved'`
写就满足了。**没有 per-enum-value 的可达性审计机制**，所以「`pending`/`rejected` 无写路径」这个
**值层缺口是 intentional 的、由本附录追踪**，不是缺陷，也无需 `audit-schema-allowlist.json` 例外
（列本身有写路径）。AI-review 流落地时 revisit 本条 + schema.ts 的 `approval_status` 注释。

> 同 §6.5 R2 的 `knowledge.approval_status` 行（init-only / UPDATE path 缺失）是同一字段的简记；
> 本附录是其 reserved-for-future 全文。schema 侧自文档见 `src/db/schema.ts` 的 `approval_status` 注释。
