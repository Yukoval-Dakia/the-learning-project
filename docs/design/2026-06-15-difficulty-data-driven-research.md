...
**Status**: Research-complete（文献研究 + 统合裁决，喂 deferred recalibration YUK-361 + ADR）
...
**Related**: ADR-0035（三维 mastery + 四引擎）· ADR-0042（MFI 调度信号，b 来源）· `2026-06-14-b1-diagnostic-engines-foundation.md`（被本研究修补 §3/§5.2/§7.3/§7.4/§8）· `2026-06-15-b1-calibration-carrier`（PR #414，本研究确认其 b 锁死是正确阶段②形态）· deferred recalibration YUK-361（本研究是其设计输入）
...
五路研究在数学骨架上**完全收敛**，无矛盾：n=1 下 b **不能**从 owner 自己的作答流被估为独立可识别量，但**能**在「外部锚定尺度 + owner 真值去偏」的半数据驱动框架内被合法更新。当前 wave（G4 红线锁 b、theta 走 Elo）是**正确的阶段②形态**，无需改动；b 的数据驱动校准是 deferred recalibration（YUK-361 阶段③）的设计输入，应在慢热**阶段③**启动，路线为 **active-PPI（IPW rectifier）+ PPI++ power-tuning**，θ 快 / b 慢靠**两时间尺度随机逼近**保证识别性。
...
- **b（慢，批量/周期）**：active-PPI 点估计，AIPW 均值 rectifier
  ```
  b̂ = (1/N) Σ_{i∈pool} m̂(x_i)  +  (1/N) Σ_{i∈labeled} [ Y_i − m̂(x_i) ] / π_i
       └──── 全候选池预测均值 ────┘     └──── HT/AIPW 残差项，对 N 归一、÷π_i ────┘
  ```
  **⚠️ 归一化修正（Codex review L87）**：rectifier 项是「对【全候选池 N】做 1/N + 已标注样本的 ξ_i/π_i」，**不是**「对已标注 n 个 ÷n 后再 ÷π_i」——后者在均匀抽样 π_i=n/N 时会把标准 PPI rectifier 多乘 N/n，系统性过度校正 b 标尺。这是 AIPW/active-PPI 的正确形式（Zrnic & Candès 2024）。
  power-tuning：`θ̂^λ = argmin [ L_n(θ) + λ(L̃_N^f − L_n^f) ]`，λ* 自适配锚质量。
- **⚠️ 真值目标量必须是难度 b，不是判分（Codex review L678，方法论硬约束）**：PPI 的标注 `Y` 必须和预测 `f(X)=logit 难度 b` **同目标量**。owner 客观题判分只给二元对错、且混入当前 θ + 学习漂移——**裸把判分当「难度真值」会把 b 校成 response-rate/θ 的混合残差，不是 item difficulty**。正解：先用**锚定 θ 的 IRT 反推难度标签**（固定该时点 θ̂，从作答反解 b 的局部 MLE）当 `Y`，或另收独立难度真值。这修正了「owner 真值去偏 b」链条的一个 gap。
- **⚠️ positivity：选题须真随机抽样 + 记录 inclusion probability（Codex review L757）**：若选题按 MFI/分歧分**确定性挑 top item**，未选题包含概率=0 → IPW/HT positivity 失效，事后归一化分数 ≠ 真实被选概率。active-PPI 要有效，选题须**真从随机分布抽样**（softmax/ε-greedy over MFI），并在**选择当时持久化 inclusion probability π_i**（非事后归一化分数）。这是 YUK-361 设计输入第 1 项的精确化。
- **识别性怎么靠它保证**：
...
| **④ 开放题外推** | propose-only | — | **无法兜底**（开放题真值非客观闭环，Y 不干净） | §7.2 G1 零文献 + §7.3 中文【阅读/语文】偏负面直证（数学/理科仍缺直证） |

### 与当前 wave 的关系

**本 wave 不改。** 当前 `item_calibration.b = LLM 先验锚 + G4 永不回写`、`theta 走 Elo` 是**正确的阶段②形态**——n=1 下 item 在线更新结构性失效，b 锁死是识别性的必要条件，不是保守。b 的数据驱动校准属**阶段③**，是 **deferred recalibration（YUK-361 阶段③）的设计输入**，不是本 wave 的实现项。
...
**「中文阅读/语文」×「开放/主观题型」的交叉格**——既无正向 r 值，又有 ZPD-SCA 2025 负面直证（Qwen-max/GLM zero-shot 评**中文阅读理解**认知难度**低于随机猜测**，且这两个模型正是本项目栈近亲）。这一格 PPI 无法兜底（开放题真值非客观闭环）。**注**：ZPD-SCA 只覆盖中文阅读理解，数学/理科/其他科目格仍是「缺直证、未验证」，不按已反证处理。
...
- **deferred recalibration（YUK-361 阶段③）的设计输入清单**（阶段③启动时需要，建议现在记进 spec，不现在实现）：
  1. 持久化每道锚题的**选中概率 π_i**（active-PPI 的 IPW rectifier 必需；当前只标「诚实边界」未给解法——这是实施层硬缺口）。
  2. `item_calibration` 未来需容纳 `b_anchor`（先验）与 `b_calib`（去偏后）分离 + `(n_i, w_i)` 元数据（阶段③才写，现在留注释占位，标明 YUK-361 阶段③ 解除）。
...
本任务为 B1 难度模型识别性 + 数学模型的文献统合裁决，**无代码 / schema 改动**（裁决结论即「本 wave 不改，纯 deferred 设计输入」）。actionable 跟进全部属既有 B1 / YUK-203 P3 calibration 子 issue 与 deferred recalibration YUK-361 的**内联订正**：

- foundation 文档 §3/§5.2/§5.3/§7.3/§7.4/§8 的上述修补点 → 追加为既有 B1 issue 的 comment；
- YUK-361 deferred recalibration 的 5 项设计输入清单 → 追加为 YUK-361 的设计输入 comment。
...
> **「用 LLM 抽题面教学特征 → 回归/树/NN 预测 item 难度 b」是一条方法学成熟、2024-26 实证活跃、且在 n=1 下结构上最稳的内容驱动 b 路线;「内容估 b 当先验锚 + 作答慢校准」的两段式是有期刊+工业级实证支撑的标准做法。唯一真天花板是 external validity:foundation §7.3 标的 r 值全部真实,但全是英文数学/阅读域 —— 而对**中文阅读/语文**不是「零直证」,是「有直证且偏负面」(ZPD-SCA 2025,只覆盖中文阅读理解,新发现,比 foundation 自标的缺口更严重)；中文数学/理科/客观题仍是「缺直证、未验证」,不按已反证处理。**
...
- **ZPD-SCA(arXiv 2508.14377,2025)** = 直接的中文 LLM 难度标定基准。**⚠️ 适用范围收窄修正（Codex review L612 + WebFetch 核实摘要）**:该 benchmark **只覆盖中文阅读理解（Chinese reading comprehension）难度分级**,**不含数学或其他科目**（本研究初稿误写「数学+语文」，已订正——数学/理科锚对它仍是**缺直证**而非「已被反证」）。核心结论:**LLM zero-shot 评估中文阅读题认知难度表现差,Qwen-max 与 GLM 甚至低于随机猜测;给 in-context examples 后改善但仍有系统性方向偏差**。
- 危险点(收窄后仍成立):(1) 它**命中本项目的中文阅读/语文这一格**(开放/主观题型重);(2) 它测的两个模型 **Qwen / GLM 正是本项目栈的近亲**(xiaomi/mimo + GLM coding plan 国产模型族);(3) 它打的是 **LLM 抽认知特征/估认知难度这一环**,正是 feature-based 路线最依赖 LLM 判断力的上游。**但严格只对【中文阅读/语文开放题】格构成负面直证;数学/理科/客观题格继续标「未验证、缺直证」,不按已反证处理。**
