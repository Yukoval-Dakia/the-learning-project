...
| `CopilotEvidenceReviewTask` | mimo-v2.5-pro | Copilot free-form turn 使用 DomainTool read 后、回复持久化/可见前（YUK-832） | 否 | — | 无工具、单轮、严格 Zod 的选稿审阅；只比较当前请求、精确 typed tool trace 与 candidate，pass 选择原字节，repair 产一份待认证安全回复，异常 fail closed |
| `CopilotEvidenceVerificationTask` | mimo-v2.5-pro | `CopilotEvidenceReviewTask` 选稿后、任何 persistence / public delta 前（YUK-832） | 否 | — | 独立从头认证最终选定字节；输入不含首轮 checks/violations/task id，只能 certify/reject、不能再写第三版；reject/异常固定 fail closed |
