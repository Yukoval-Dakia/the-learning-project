...
| 任务 | Provider/Model 选择 | 触发 | tool call | 多模态 |
| --- | --- | --- | --- | --- |
| `VisionExtractTask` | 低成本视觉（CMMMU 选型） | 录入错题图片 → 文字 | 否 | 输入 |
| `VisionAnswerExtractTask` | 同上 | 答案图片 → 文字（pipeline 路径） | 否 | 输入 |
| `AttributionTask` | Sonnet → Haiku 备选 | 错题归因 + 挂载知识点 | 是 | — |
| `VariantGenTask` | Sonnet + batch | 变式题生成（draft 状态） | 否 | — |
| `QuizGenTask` | Sonnet (+ batch 可选) | embedded / daily / final / 用户主动 | 否 | — |
| `JudgeRouter` | n/a | 答案提交后路由 | 否 | — |
| `JudgeExactTask` | n/a | exact judge | 否 | — |
| `JudgeKeywordTask` | n/a | keyword judge | 否 | — |
| `JudgeSemanticTask` | Sonnet / Haiku | semantic judge | 否 | — |
| `JudgeRubricTask` | Opus / Sonnet | rubric judge | 否 | — |
| `JudgeStepsTask` | Sonnet | computation 步骤验证 | 否 | — |
| `JudgeMultimodalTask` | Opus / GPT-5.x (multimodal) | image 答案 + 高 visual_complexity | 否 | 直接 |
| `JudgeFlexibleTask` | Opus / 顶级 reasoning | ai_flexible 兜底 | 否 | 视情况 |
| `WeeklyReportTask` | Opus + prompt cache | 周复盘 | 是 | — |
| `DreamingTask` | Opus + batch + prompt cache | 夜间生产 lane | 是 | — |
| `MaintenanceProposeTask` | Sonnet + batch | 维护 lane 提议 | 是 | — |
| `NoteGenerateTask` | Sonnet + batch (atomic) | 学习意图触发 | hub 是 | — |
| `NoteVerifyTask` | 不同 model + batch | Note 双 pass 反幻觉 | 否 | — |
| `NoteSectionUpdateTask` | Sonnet | Living note 更新某 section | 否 | — |

每个任务独立选 provider，自由用 provider 特色（prompt caching / batch API / structured output / multimodal），上层只关心业务语义。
...
LearningItem                      // 待学习列表
  ...
  priority: int                   // hybrid sort: 用户 pin (high) > AI score
  user_pinned: bool
...
Artifact                          // Note 优先；Tool 型 Phase 3 评估
  ...
  sections: [{                    // atomic
    ...
    user_verified: bool
    embedded_check?: { question_id }   // 引用 Question 表
    version
  }]
