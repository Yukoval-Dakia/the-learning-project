...
> owner maps 当前保存 50 个完整 TaskSpec 与 1 个显式 transitional entry，registry 只做静态 compatibility projection；
> transitional semantics 只剩 `legacy-task-definitions.ts` 中的 `SessionSummaryTask`；其 owner migration 由 YUK-870 承接，
> 中央 semantic quarry 最终删除由 YUK-885 承接。
...
> 保存 staged semantic ownership：50 个完整 owned TaskSpec（18 Practice + 3 Notes + 8 Ingestion + 3 Knowledge + 13 Agency + 5 Copilot）和 1 个
