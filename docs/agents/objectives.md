...
- **工具面**：`allowlists.ts` 的 `COPILOT_TOOLS`（全部安全 read + `propose_knowledge_edge`）；`copilot_user_suggested_mistake_action` 变体在用户显式触发时追加 `attribute_mistake` + `propose_variant`。*Planned*：AF §3.1 授予 `search_memory_facts`（coach/dreaming/copilot 三编排角色之一）。
- **边界**：
