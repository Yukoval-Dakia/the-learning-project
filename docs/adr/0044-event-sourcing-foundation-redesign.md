...
- 🔴 **头号硬前置 — worker 进程 `ctx.causedByEventId` 透传**（ADR-0041:55 自认 half-wired）：durable copilot job 跑在 worker，若不透传 user_ask event id 进 ToolContext，durable run 内所有 mirror/propose 事件 caused_by=null → 整条下游脱链、级联漏收。**优先级高于一切 rename。**
