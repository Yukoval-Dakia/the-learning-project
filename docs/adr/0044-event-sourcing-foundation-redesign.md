...
- ✅ **worker manifest 接线（YUK-328，2026-07-18）**：完整 37-tool inventory 已由 capability manifests 派生；API 在 listen 前、独立 worker 在注册 handlers 前 await 同一 `registerCapabilityTools` interface。请求期 `registerCoreTools` / `CORE_TOOLS` bootstrap 已退役，真相源对齐缺口关闭。
...
## Addendum — YUK-561（2026-07-04）：θ̂ revert-bracket decide-and-record

**Status**: Accepted（owner 2026-07-04 AskUserQuestion）。**Decision source**: `docs/design/2026-07-04-revert-bracket-spec.md`（reconciled 终稿 + O2 双 sibling 对齐）。**Unit**: master register polish-order #10（`G3/F8 revert-bracket`，P1-systemic）。

**问题（register F8）**：本 ADR §Wave 0 落地的 A-class `state_snapshot` 在生产 topology 下**结构性不可达**——snapshot 挂 attempt event `E`（`caused_by=null` → `E` 恒为 cascade root → `classifyRow(E)='irreversible'` → step-4 在 restore 前 refuse）。三条 live θ̂/mastery 写路径都隐性押注一张既没接线又打不着的 revert 网。

**决定**：

1. **方向 A（修可达 + 接线）**，退休 register 选项 (b)（θ̂/mastery 记 append-only + 退 A-class）——owner：θ̂/FSRS 不应是全系统唯一残留的结构性单向写面；#9 记忆层可逆先例同向。

2. **可达性修复 = 中间 grading-checkpoint 锚**（S2）：attempt `E` → checkpoint `C`（`caused_by:E`）→ snapshot（`caused_by:C`）。`revert(C)` 闭包 = `{snapshot}`，`E` 是 `C` 的父、reverse-CTE 不上爬 → snapshot 可达可逆。checkpoint 归 `EVENT_LAYER`（`correct(retract)` 即完整反转，无独立 SoT 行）。

3. **O2 完整案——θ̂/FSRS 双 sibling checkpoint**（owner 推翻轻量单快照+段过滤）：每 attempt 按段各写独立 checkpoint+snapshot（`C_θ`→`S_θ` / `C_f`→`S_f`），一个 `experimental:grading_checkpoint` action 承两段（`payload.segment` 判别）。**段撤销 = 撤哪个 checkpoint**（无 `revertSegments` 过滤、orchestrator 无段裁切、retract 无 `reverted_segments`——段身份由被撤 snapshot 的 subject_id 自证）。段独立 collect/guard/tombstone、永不需存量迁移。代价（登记面/双写/双事件原子性）owner 接受。

4. **snapshot 无损化**（S1）：θ̂ `before` 从「只 theta_hat」扩为**全部 attempt 可写列**（counts/precision/delta/last_outcome_at/rt_correct_ms/theta_grid_json）的 union（rich | legacy bare number | null）——restore = verbatim 全行还原（堵住 pre-S1「θ̂-only + counts 归零 + precision 陈旧」失真）。SRT_ENABLED live → rt 列今天就写。legacy bare-number → restore typed refusal `legacy_snapshot`（绝不 lossy）。列漂移守卫测试锁 mastery_state ↔ ThetaRowSnapshot lock-step。

5. **首个 live caller = rejudge overturn（θ̂ revert-only）**，三条对抗审查必修：
   - **原子性（Q2a）**：newJudge + correction + revert + marker 同一 `db.transaction`——改判与 θ̂ 自愈同生共死。修 pre-S4 失效链（非原子 → revert 瞬时失败 → pg-boss 重试 → 幂等守卫命中已 commit 的 newJudge → `already_resolved` → revert 永不再跑 + 假完成）。orchestrator `db: DbLike` tx-aware（step-6 SAVEPOINT 嵌套、conflict/legacy 以返回值回 caller）。
   - **双触发门（Q2b）**：θ̂ revert 只在 `judgeDriven`（attempt 恒真 / review 需 `payload.judge.auto_rated===true`——手评 θ̂ 撤它是纯污染，family-calibration 同源判例）**且** `bitFlip`（`coarse∈{correct,partial}?1:0` 位翻转——partial↔correct 不撤合法信号）时触发。不满足 = 零动作零 marker（O3 owner 拍零 marker）。
   - **refusal 分型（Q2c）**：orchestrator 新增 `no_checkpoint`（rootRow null——旧 attempt/该段未动，honest defer 信号，非 caller bug）；`irreversible` 收窄为「闭包含真 learner fact」。四态分派（无死 fallback）。

6. **残留全量可见（Q2d）**：revert-only 只抹错误 transition、**不 re-apply 正确 outcome**（第二实例原则）——**happy-path 成功 revert 也写 `experimental:reproject_deferred` marker**（residual=reapply），第二实例重投影引擎 worklist 完整（register「make the residual visible」）。全历史重投影 = 独立 issue。

7. **归因订正（Lens A F4）**：防**双 revert** 的是**冲突守卫**（revert 后 current=before≠after → conflict refuse），=LIFO 正确性载体（restore-to-before 只对该 KC 最近一次 transition 数学正确）——**不是** `correct(retract)` tombstone（cascade 只排除 correct 节点本身，snapshot 仍被 re-collect）。tombstone 是 fold/读层墓碑。注释 + 本 addendum 按此归因。

8. **append-only 边界**：原 snapshot/checkpoint/attempt/judge 永不删；revert 只 state-table 还原 + 追加 `correct(retract)` + marker。checkpoint/snapshot/retract/reproject_deferred 全 `ingest_at:now`（内部账本，跳 memory outbox）。**本波无遥测 event**（draft 的 `cascade_revert_applied` 砍——成功可观测性由 retract 行 + marker 查询覆盖）。

**新 experimental actions（2 个，均 reserved + 专属 schema，O1）**：`experimental:grading_checkpoint`（可逆锚）+ `experimental:reproject_deferred`（残留 marker）。红标：`grading_checkpoint` schema/RESERVED 与 writer 同 commit（每 attempt tx 写它，barrier miss 崩全线）。

**下游解锁**：cascade-revert-taxonomy（随本波，RESHAPE 核心）· judge-spof（半随——revert-on-overturn 本波，完整 reprojection 独立）· elo-k-schedule（工程半随——count+precision restore 修复）· conjunctive-multi-kc-credit（per-KC 随本波——全列捕获关 per-KC gap）。

**运维**：见 `docs/runbooks/revert-bracket.md`（手动 revert / refusal 四态处置 / 双 sibling 撤段 / 多 KC defer 预期）。

**红线遵守**：n=1 不拟合 item 参数（捕获/还原是学习者自状态充分统计）· fold-owned 表不 raw UPDATE（restore 走 upsert single-writer，DELETE 是冷启 INSERT 的逆同 tx）· 数据门只 gate 翻转不 gate build（机制全建 + caller 穿 live；A2/FSRS 只 defer flag 半边；grid/rt 列现在就捕获）· 护栏两层（硬顶 depth/cap/守卫已在；warning 水位 = 观测查询）· 无死 fallback（refusal 四态 + legacy 全显式；happy-path 残留也可见）。
