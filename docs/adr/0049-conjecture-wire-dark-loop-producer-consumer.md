...
### 2. Q2 — Probe answer 入口：隔离 judge registry 路径 + A5-a outcome→resolution 分流

新 route `POST /api/conjecture/probe/[id]/answer`（token gate，agency manifest）。**ND-5 隔离靠物理路径**：judge 路由走 `defaultJudgeKindForQuestion` → capability registry `resolveJudge` → `runner.run()` PURE——**NOT** `submit.ts` 的 `createDefaultJudgeInvoker`（attempt 域耦合 wrapper）。registry 直调路径**结构上触不到 FSRS/attempt 写**。唯一写是 `answerProbe` 的单个 `experimental:probe_result` event。
...
| 红线 | 守法 | 证据 |
|------|------|------|
| **ND-5**（probe 不写 FSRS/attempt/θ̂） | judge registry 直调（非 invoker）+ answerProbe 只写 probe_result event | `probe-answer.ts` import getDefaultRegistry/resolveJudge（非 createDefaultJudgeInvoker）+ `probe-lifecycle.ts:194` 注释 |
