...
**Supersedes:** —
**Superseded by:** 部分迁移 — YUK-321 M5（2026-06-13）：共享后端 Redis → PG 表 `editing_presence`，见文末「M5 迁移注」
...
## M5 迁移注（YUK-321，2026-06-13）

旧栈拆除（M5 Task 9，gate 选项 b）把共享后端从 Redis 换成 PG 表 `editing_presence`
（`src/server/artifacts/presence/pg.ts` 的 `PgPresenceStore`，`editing-session.ts` 恒用之；
in-memory impl 仍是 fast unit 测试默认，经 `vi.mock` 注入）。子计划见
`docs/superpowers/plans/2026-06-13-yuk-321-pg-presence-subplan.md`。

**继续成立的核心决策**：

- `PresenceStore` 抽象与全部状态机语义（首戳、sticky idle、force-apply ceiling、FIFO flush）；
- 跨进程共享 presence（web 写 / worker 读）——YUK-148 的修复本体；
- 「决策原子、DB IO 在外」：Lua 单线程原子段的 PG 等价物 = 短事务 + `SELECT … FOR UPDATE`
  行锁；`persistNoteRefineApply` 仍在原子段之外；
- ephemeral 语义：Redis TTL 自然蒸发的等价物 = load 时丢弃 `queuedAtMs` 超过
  `EDITING_FORCE_APPLY_TIMEOUT_MS`（10min）的陈旧 pending 并 `console.warn` 留痕。

**不再成立**：

- `redis:7-alpine` 容器、`REDIS_URL` 选择逻辑、`ioredis`（依赖与 `src/server/redis/` 均已删）；
- YUK-171 的 fail-safe 降级（try-catch → idle）**不移植**：PG 即业务库——PG 不可用时
  `persistNoteRefineApply` 同样写不进去，降级决策无意义，错误直接抛，与仓库其它 db 路径一致。

**路径迁移**：本文提及的 `app/api/editing-session/*` route 已随旧栈拆除迁移至
`src/capabilities/notes/api/editing-{heartbeat,blur}.ts`，由组合根 `server/app.ts` 挂载。
