...
> 3. **worker `ctx.causedByEventId` 透传 = 头号硬前置（优先级高于 propose_* rename）**：durable copilot job 跑在 worker，不透传 user_ask event id → durable run 内 mirror/propose 事件 caused_by=null → 整条下游脱链、级联漏收。YUK-363 Wave 0 落地。
...
- **worker 面 copilotTools 只半接线（half-wired gap）= 硬前置，reach+endurance 双重承重**:durable copilot job 跑在 worker、要全套 copilotTools surface,worker 仍靠 bootstrap 全量注册、manifest 贡献制未覆盖 worker 进程——实施波次第一个要补。
