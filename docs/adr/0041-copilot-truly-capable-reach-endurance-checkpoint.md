...
- **生成型 reach = `run_task` dispatcher**:每个 copilot-invocable 生成任务在 registry 声明 `{ intentSchema（题型/形态无关的意图）, prepare（任务专属预处理：id 校验/profile 解析/归一，不可被 bypass）, invocable flag }`。类型复杂度（如 9 种 QuestionKind）被 typed task + Zod 吸收,**copilot 只给意图**。加新任务进可达范围 = 声明 3 字段,非今天多处手接线（surface allowlists + 包 manifest + 测试计数 + 工具文件）。
...
- **worker 面 copilotTools 半接线已关闭（YUK-328，2026-07-18）**：manifest 现覆盖完整 37-tool inventory，API/独立 worker 都在接流量或任务前 await `registerCapabilityTools`；请求期 CORE_TOOLS bootstrap 已删除。`causedByEventId` 透传见上方 §1 amendment 第 3 点——同样已闭环。
- `propose_*` 命名债:8 个直接结构化写在 live+revert 下已是「live 写+可撤」,`propose_*` 成误名,实施波次统一 rename（连同 allowlists/manifest/测试计数）。
