...
**Related:** ADR-0053 · ADR-0054 · ADR-0055 · ADR-0052 · YUK-842 · YUK-575 · YUK-927 · YUK-757 · YUK-920 · YUK-837
...
### 2. Retire mailbox continuation from the foreground main path
...
- If native Task replaces `launch_researcher` on the model surface, say so explicitly in impl; do not expose
  two parallel result channels.

### 3. Keep the SubagentRuns noun until B3
...
- Do not drop `get|wait|cancel_subagent` projection if Dock still needs it.
...
- Do not treat the child as a nested DomainTool borrow of a released previous POST. One POST = one family, as
  ADR-0054.
- Child that overruns remaining 90s HTTP budget fails closed; do **not** extend `HTTP_PROVIDER_SESSION_BUDGET_MS`.
...
- `src/capabilities/copilot/jobs/copilot_continuation.ts` and `copilot_run.ts` (`runCopilotContinuationTask`
  foreground path)
- `src/capabilities/copilot/server/tools/subagent-controls.ts` (model surface if `launch_researcher` retires)
- Foreground acceptance tests: child blocks parent; native `tool_result` in same query; no
  `copilot_continuation` job for foreground inline chat; no synthetic root continuation turn.

Foreground acceptance: a `launch_researcher` / native Task child that runs to completion returns
`tool_result` in the same live parent query; no pending `copilot_continuation`; no new logical CopilotTask for
...
- Child result is native Task `tool_result` in the same SDK session. Parent holds the live `WarmQuery` until
  child settle or deadline. Reconnect after POST death is ADR-0054 resume only.
- `subagent_run` noun, control tools, and Dock projection remain until B3; only the mailbox continuation path
  goes away on foreground inline chat.
...
- Remove automatic `copilot_continuation` → `runCopilotContinuationTask` for foreground inline chat.
- Keep `subagent_run` noun and projection; retire continuation as second result channel only.
- Rewrite tests listed in pin 7. Foreground acceptance above is the scoped gate.
