...
4. **epistemic 纠错复用 `CorrectionKind`（ADR-0019），不引 valid-time**。结构「不再为真」走 correction event（`CorrectionKind` ∈ {supersede, retract, mark_wrong, restore}，已有 `getEffectiveTruth` event-层实现，`core/schema/event/known.ts`）表达知识论纠正，不再用 `valid_at`/`invalid_at` 表达时段有效性。一条边被撤 = retract/supersede 一个纠错 event，不是给边盖 `invalid_at` 时间戳。
