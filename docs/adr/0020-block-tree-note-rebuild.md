...
采纳 **Y 路径**（原 §3.C 复活但收敛）：atomic 也变 block tree，三态共用 schema field，旧 `sections[]` / `outline_json` / `child_artifact_ids[]` 全 DROP。本 ADR 锁核心架构契约；具体 TipTap PM node schema 在 P2 实现后补 ADR-0022。
...
### 5. Cross link 物理形态（L3+L2 索引混合）

- **L3（source of truth）**：P2-basic 的具体 node 形态见 [ADR-0022](0022-tiptap-pm-node-schema.md)：`crossLinkBlock` 携带 `attrs = { artifact_id, block_id?, title? }`；future inline cross-link mark / node 也必须保留同一 anchor attrs
...
- artifact 写路径同步扫 body_blocks 抽 `crossLinkBlock` / future cross-link attrs → upsert 索引表
- 反链查询走索引表 O(log N)
...
- atomic body_blocks 内一个 `artifactRefBlock` block（`attrs.artifact_id` + `attrs.artifact_type = 'tool_quiz'`）inline render；具体 node contract 见 [ADR-0022](0022-tiptap-pm-node-schema.md)
...
- 具体 PM node schema（SemanticBlock / CrossLinkBlock / ArtifactRefBlock / CalloutBlock / AutoLinksContainer）在 P2 实现后补 **ADR-0022**；本 ADR 不锁
...
- **ADR-0022（P2 后补）**：TipTap PM node schema 具体形态
...
- **P3 + P4 并行 schema 撞**：ADR-0022 锁 PM node schema 后才可并行
...
## Amendment — YUK-384 durable hub-sync reconciliation (2026-07-21)

§9 的 hub AutoLinksContainer auto-zone 收敛从「best-effort 夜间直接 apply」改为
**PostgreSQL-authoritative、generation-fenced 的 desired-state reconciler**。变更点：

- **不变量保留**：§9 的确定性 auto-zone 构造（`resolveHubMeshAtomics` +
  `buildAutoZonePatch`）逐字保留，只是现在跑在 reconciler 的 fenced apply **内部**，
  不再由夜间 job 直接写。`attrs.suppressed_block_refs[]` dismiss 语义不变。
- **触发器所有权**：`knowledge` / `knowledge_edge` / hub-local `artifact` 拓扑写在
  同一事务内经触发器推进 `hub_sync_reconciliation.generation`。收敛靠触发器
  （耐久）+ 每分钟恢复地板（≤60s），nightly 退化为幂等覆盖修复扫描——**无 scheduled
  路径直接 apply**。
- **编辑并发**：editing presence 改为 session-qualified（`artifact_edit_session`
  按 (artifact, editor session) 键），finalize 在共享 advisory lock 下按 session
  判活跃（恰 30s 活跃，> 30s 过期）；删除 §（旧）10 分钟强制 apply。non-hub
  note-refine defer 队列（`editing_presence.pending`）语义原样保留。
- **rollout / 运维 / 所有权守卫 / 即时唤醒接线**：见 `docs/architecture.md` §十.1
  （`HUB_SYNC_MODE` off→shadow→apply 八步序、operator 告警阈值、
  `GET /api/admin/hub-sync`、`pnpm audit:hub-sync-writers`）。**provisioning 必须
  `pnpm db:migrate`（触发器/函数/partial index 只在手写 0071 SQL，db:push 会产出
  有表无触发器的死库）；`triggers_installed` 健康字段是运行时哨兵。回滚：`HUB_SYNC_MODE=off`
  只停 reconciler，触发器记账仍继续——若回滚因触发器本身，紧急停用要 `DROP TRIGGER`
  三条（重建 = 重跑 0071 CREATE TRIGGER 段 + 一次 nightly repair），详见 §十.1「off 语义」。**
- **决策 D1-D9 / 完整设计**：`docs/superpowers/plans/2026-07-21-yuk-384-durable-hub-sync.md`。
