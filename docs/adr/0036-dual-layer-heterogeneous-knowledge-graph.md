...
4. **RT2 层级 credit 传播 = 派生量，不物化回边**：从被 attempt 的高阶知识点沿 `prerequisite` **反向**遍历（to→from），`weight × encompassing_weight` 连乘衰减，递归 CTE 算出 implicit evidence 喂 mastery 派生层——**不写回 `knowledge` 行**（与 ADR-0012「mastery 住 view」同源）。
