...
**Partially superseded by:** [ADR-0047](0047-kg-load-bearing-borrow.md) for Decision 4 (RT2) only. The dual-layer graph decision and Decisions 1–3, 5–6 remain in force.
...
**Related**: ADR-0010（knowledge mesh：tree backbone + typed cross-edges + experimental escape；本 ADR **扩展并收紧**其 relation_type 治理，把 5 核心 enum 钉为闭集、weight 钉死 confidence-only）· ADR-0006 v2（event 核：错因现活 event 层 `action='judge'` payload，本 ADR 据此把误区身份层与观测层显式分离）· ADR-0012（mastery 派生 view；本 ADR 的历史 RT2 credit 方案同样不写回边，现行机制见 ADR-0047）· ADR-0028 / ADR-0030（知识级 FSRS + by-kind 选题：误区节点不持独立调度，remediated_by 复用 FSRS 管线）· ADR-0034（写入期结构一致性闸地基，YUK-344；本 ADR RT1 晋升环 / RT4 类型签名闸 gated 在其之后；RT2 的当前前置改由 ADR-0047 定义）。
...
4. **RT2（历史决定）层级 credit 传播 = 派生量，不物化回边**。

   **Superseded for implementation and mechanism semantics by ADR-0047.** The historical encompassing_weight/p(L) credit design below is retained as decision history and is not the current runtime contract.

   历史设计从被 attempt 的高阶知识点沿 `prerequisite` **反向**遍历（to→from），**沿 `encompassing_weight` 连乘衰减**算出 implicit evidence 喂 mastery 派生层——**不写回 `knowledge` 行**（与 ADR-0012「mastery 住 view」同源）。
...
**正面**
- 关系结构的目标形态从「三张平行图」收敛为一座双层异构图，且全部 **Postgres 无图库可承载**——零新基建、零新引擎（决策总账 §综合主线）。现行身份层增量为「一张并行表（misconception）+ 一张异构边表」，关系治理增量为一个脚本（`audit:relations`）；历史 RT2 的 `encompassing_weight` 列不再计入当前目标形态。
...
- RT2 的现行 implementation/mechanism contract 见 ADR-0047；本 ADR 的 `encompassing_weight`/p(L) credit 方案仅保留为历史决定，不再描述当前 runtime。
...
- **RT2 历史成本（不再是当前实现要求）**：`encompassing_weight` 加列原会触发 `audit:schema` 并要求 propose 写路径；ADR-0047 的现行契约不授权该 schema/audit/runtime 工作。
- **RT1/RT4 数值阈值**（晋升 k、频次 N≈15-20）单用户无基线，须埋点观测 N 周——观测窗口期内误区图 / promote 不可用是特性非 bug（决策总账 §5；Phase 2 §5.3），owner 须接受空窗期。历史 RT2 的 `encompassing_weight≈0.3-0.4` / credit 阈值不再具规范性；当前 borrowing 阈值、证据闸与 disposition 全见 ADR-0047。
- **开放题 observed_in 证据精度退化**：文献默认 MCQ distractor 级归因，本项目古文开放题靠 judge cause 归因更软（B1 有效性天花板）——开放题为主科目的误区图实际可用性存疑，需打样数据集实测。
- **环形依赖盲区**：caused_by 方向语义要靠一致性闸地基（ADR-0034）钉死，而闸本身代码侧零实现——这是 RT1/RT4 的当前硬前置，本 ADR 的身份层增量在闸就位前不得起跑。历史 RT2 的旧前置不再具规范性；当前 RT2 前置见 ADR-0047。
...
- **RT2 历史备选**：新建 `encompasses` 第六 relation_type / encompasses 边表，或把 credit 物化回 `knowledge` 行——当时否决：credit 是派生量（ADR-0012 同源），历史替代案为复用 prereq 反向遍历 + 一 nullable 列；该替代案现已由 ADR-0047 supersede。
