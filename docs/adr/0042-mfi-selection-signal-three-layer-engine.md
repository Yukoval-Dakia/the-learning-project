...
L3 全部职责的穷举；§5 的学习 mix 质量约束仍归确定性 L3，但按风险分成两种强度：

- **fatigue/repetition 是确定性 repair**：`post-filter.ts` 以先验阈值 2 限制连续同一主 KC /
  同一题型。它只做稳定重排，不改变流成员、不改变 sampler π_i，并保持 due 子序列逐项一致；
  paper 切断连续段。无合法替代时不删到期题或抽中题，保留原项并产
  `fatigue_repetition_unresolved` 诊断。
- **「每日 ≥1 frontier/transfer」暂时 re-scope 为 observable floor，不是 hard membership
  invariant**：当前确定性供给只有 frontier，没有 transfer source；稀疏先决图、KC 无 active
  题、时间预算和随机 sampler 都可能让最终流没有 frontier。L3 不得发明题，也不得在抽样后
  强塞候选并继续沿用旧 π_i。因此无 frontier 时产 `anti_comfort_floor_unmet`，区分
  `no_candidate` / `not_selected` / `budget_truncated`，并由物化入口发结构化 observability
  log。它仍是必须被看见和治理的产品退化，不再是静默 NO-OP。
- **升格条件**：YUK-361 Phase 3 接入确定性 transfer 候选与合法的 inclusion-probability
  处理后，fallback 顺序才可升级为 `transfer → frontier → diagnostic` 的 hard floor；届时另写
  amendment，把 `anti_comfort_floor_unmet` 从告警收紧为仅供给耗尽时才允许的例外。

这段 reconciliation 显式取代 §5「全进 L3」可能被读成「今天即可无条件强制成员」的最强
解释，同时保留 owner 决策总账 ⑥⑦ 的方向：fatigue 今天确定性执行，反舒适退化今天必须可观测，
完整 ≥1 保证待真实 transfer 供给与统计契约齐备后升格。
...
## 2026-06-26 Amendment — 候选隔离不变量（X §4.4.4 印证）+ 选题阶段契约

承 X 算法深潜（`docs/research/2026-06-25-x-algorithm-deep-dive.md`）→ 融合设计（`docs/design/2026-06-25-recommendation-engine-x-algorithm-mapping.md` §3.1/§3.6 + 决策 a/e）。owner 2026-06-26 拍板：把候选隔离收为本 ADR 显式不变量并补缺失测试；选题阶段做文档化契约、**拒**泛化成通用 trait 框架。

**候选隔离不变量（命名，scoped 到基值打分层）**：candidate-signals **基值打分层**（`mfiScore`/`klpScore`/`diagnosticScore`，`src/core/selection-signals.ts`）对每个候选的分 **MUST 是 `(θ̂, b, precision)` 的纯函数 + batch-order/membership/size 无关**（可缓存、replay-safe）。跨候选耦合**只允许**在三个明确命名的下游层——① **确定性后置乘子层**（多样性/疲劳/OON 几何衰减，YUK-370）② **IPPS-sampler**（`softmaxProbabilities`，减 batch max / 除 batch total，本就 batch-耦合）③ **LLM 编排层**（`selection-orchestrator`，2026-06-15 amendment「让 LLM 强」故意整批 prompt + batch-aware 权重）。**θ̂-更新路显式排除**：`HIERARCHICAL_ELO_ENABLED=true` 下 `updateThetaForAttempt` 每 attempt 还会按 domain 漂移共享 `θ_global`（θ̂=θ_KC+θ_global，YUK-466，`theta.ts:173`/`state.ts:712-714`）——故不变量只锚定**读/打分路**，**不**锚定写态的更新路。

**这镜像 X 自己的分层**：X PhoenixScorer（候选隔离 / 可缓存）⊥ home-mixer RankingScorer（多样性/OON 后置乘子，非隔离）。X §4.4.4 在重 transformer 里**处处强制**全链隔离以求规模可缓存；我们**故意分歧**——只把隔离 scoped 到基值层，在三个下游层确定地或自觉地耦合换 batch-aware 教学法（n=1 无 X 的规模/缓存压力，单学习者 + 夜间 compose + 廉价增量重排）。这不是 bug 是自觉权衡（同 §10「借架构纪律、拒数据前提哲学」）。

**诚实前提 + 真正的活**：我们**今天就有** per-candidate 纯基值函数（标量入标量出），但**此前无 batch-invariance 测试**——`assertL3Invariants`（`softmax-selection.ts:540`）断的是另一族铁律（due-presence / due-order / recall-lock / capacity，即四铁律），**从不**断基值打分纯性或 batch-order 无关性。故本不变量的「活」= 补 guard 测试:`src/core/selection-signals.test.ts` 新增「候选隔离不变量」describe（membership/order/扩容三组 `Object.is` bit-identical 断言 + sampler-层故意耦合的边界断言）。X 自己 `test_recsys_model.py` 也只测 mask 结构、从不断跨候选 logit 相等——我们补的正是这条端到端断言。该测试也是 YUK-493 同构核 bit-exact replay 的回归地基。

**选题阶段契约（决策 e）**：现有 `composeSoftmaxStream` 已天然是 `Source → Hydrator → Filter → Scorer → Selector → SideEffect` 分阶段编排（镜像 X candidate-pipeline §4.2）。本 ADR 文档化此**阶段契约**（含上面的候选隔离不变量分层），**但拒**把它泛化成通用可复用 trait 框架（X §4.2）——X 需通用 trait 因它跨服务网格组合多条 pipeline（ForYou/ScoredPosts/ads/WTF），我们只**一条**日流，泛化=零第二消费者的间接。保留具体 `composeSoftmaxStream`。
