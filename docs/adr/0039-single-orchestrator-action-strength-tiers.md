...
1. **单编排者 D14 = 前台 Copilot + 后台 4 job 合为同一编排者的不同召唤姿势**。前台 Copilot（`actor_ref=copilot`）/ 夜链 dreaming（`actor_ref=dreaming`，产交班缕）/ coach daily+weekly（`actor_ref=coach`）/ goal scope（`actor_ref=goal_scope`）是**同一个 D14 的四个召唤姿势**，对用户讲一条连贯故事——前台对话能引用昨夜后台想的（A1 交班缕），后台 job 写会话级工作记忆供前台读。**合为一个叙事 ≠ 丢可观测性**：留痕仍按 `actor_ref` 分轨（evidence-first / runs log 到 `src/server/ai/log.ts` 延续）。这 **partial-supersede ADR-0029 §决定 4** 的「四 agent 各持独立人格 surface」那一面——ReviewPlanTask / Coach 不再是互不知情的独立人格，而是 D14 的召唤姿势；ADR-0029 的实质**治理意图**（不读记忆、checkpoint 自适应不让编排者进 FSRS 热循环、FSRS *when* 独立真相源）被合并引擎/D14 **继承延续**。**注（修与 ADR-0037 的矛盾）**：`review_plan` job 本身已按 **ADR-0037 退役**（不再独立注册/另开 paper channel），故这里**不是「保留 review_plan 窄面」而是「其治理意图迁入合并引擎」**——别照字面去维护一个 ADR-0037 要消除的 review_plan surface。其余仍存活的自主 task 窄面（如 `ingestion_block_edit`）经 ADR-0032 面级 allowlist 保持窄 + 同时授予 copilot（D0-2 RP-5）。

2. **全局出手强度表 A/B/C（统一出手契约，按「可逆性 × 后果」分档）**，管 D14 对所有面（inbox kind / mem0 extraction / inline 动作 / Today 交班缕 / 练习流提议）的出手：
   - **A 档（自动 + 撤销窗口）**：乐观应用、不打断不强制确认、后台静默 apply + 顶部可撤销提示 + 一键 revert，不进 inbox lane。落 mem0 episodic 客观事件 + 真正干净可撤的轻量 kind。健康指标 = **revert 率**。（**修 review 发现的误归档**：`record_links` **不归 A**——`acceptRecordLinksProposal` 改 `learning_record` 的 `knowledge_ids`/`question_id`/`learning_item_id`/`artifact_id`/payload 多字段，非干净单步可撤，归 B 档；归档默认偏保守见决定③。）
   - **B 档（逐条人审）**：结构性 / 破坏性写入逐条裁决，走现状 inbox accept/dismiss lane。落 knowledge_node/edge/mutation、variant_question、learning_item transition、relearn、goal_scope、block_merge、question_draft/edit、**record_links**（多字段改 learning_record）+ mem0 semantic-trait（偏好/习惯/弱点）。健康指标 = **dismiss 率**。
...
## Amendment（2026-06-15）—— A/B/C 18 行归档表拍定（owner「全保守」裁定）

**Source**: `docs/design/2026-06-15-rethink-implementation-gate.md` §1.6 + owner 2026-06-15 裁定。代价/风险节「为每个 inbox kind 标 A/B/C 一张 ~18 行归档表（Phase 2 收口拍）」于此落定。对着 `aiProposalKinds`（`src/core/schema/proposal.ts:6`，18 个 kind）逐行按「可逆性 × 后果 + 拿不准归 B + 熔断兜底」赋值：

| kind | 档 | 依据 |
|---|---|---|
| knowledge_node / knowledge_edge / knowledge_mutation | **B** | 结构性创建/改动 |
| learning_item | **B** | transition |
| variant_question | **B** | 入 active 池 |
| relearn | **B** | 调度态改 |
| record_links | **B** | 多字段改 learning_record（决定 #2 已记从 A 改回 B） |
| record_promotion | **B** | 记录升格结构态 |
| goal_scope | **B** | North-Star 实体物化 |
| block_merge | **B** | 合并切块，硬安全边界无 auto-merge |
| question_draft | **B** | accept = promote draft→active + FSRS enroll |
| question_edit | **B** | active 题 structured node edit |
| completion | **B** | 干净可撤是唯一 A 候选，保守归 B；埋点 revert/dismiss 率后可迁 A |
| note_update | **A（小改·不触 user_verified）/ B（大改或触 user_verified）** | **ADR-0040 决定1 精化**（终档已拍）：按可逆性 + user_verified 拆档——小可逆改（`ops≤3 且 new_blocks≤2` 且不触 user_verified）走 A 档自动应用（补熔断 + 统一 revert UI + user_verified 硬边界），大改或触 user_verified 走 B 档 propose |
| image_candidate | **B** | accept 触发**付费 VLM 抽图**（ADR-0002 用户授权付费动作），绝不 A |
| defer / archive / judge_retraction | **C** | 移出裁决面（决定 #4）：snooze 控件 / 直接软归档 / agent-notes 旁观 |

**结果：15 B + 3 C + 0 A**。这 18 个 inbox proposal kind 在保守判据下**无一进 A 档**——A 档（自动 + 撤销窗口）只落 **mem0 episodic 客观事件**（非 inbox 轴）。inbox 提议天然需裁决，这是「拿不准归 B」的诚实落点。`inbox-meta` 单测须钉住此映射（增删 kind 时漂移被拦）。
