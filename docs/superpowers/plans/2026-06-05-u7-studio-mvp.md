...
| `audit-profile.test.ts:36` asserts `.toEqual(['math','physics','wenyan'])` (hardcoded) — does **not** import `auditSubjectProfiles` directly anywhere in the file | `scripts/audit-profile.test.ts:32-37` | Q2/RL2: assertion must dynamic-ize; const export safe to keep |
...
Critic is invoked through the compile CLI via a `--critic` flag (chosen over a standalone `scripts/critic-profile.ts` — keeps the draft-load + parse path single-sourced; the CLI already reads & validates the draft, Critic just adds an LLM pass on the same parsed object). The CLI calls `runAgentTask('ProfileCriticTask', input, { db, allowedTools: [], subjectProfile })` with:
- **real db** — preserves the ai-run trace (`task_run_log` + `cost_ledger`), per evidence-first (`runTask` writes both unconditionally; §0.1). This satisfies the spec §4 "agent tasks are traceable" requirement even though §4 prose said "no DB". The Map C1 tension resolves here: standard `runTask` *must* write DB, so we embrace the trace rather than fork a no-log runner.
- `allowedTools: []` — single-shot, no tool loop (`needsToolCall:false`, `maxIterations:1`, `timeout:60_000`; mirrors `SolutionGenerateTask`/`MemoryBriefTask`).
...
- **Modify** `scripts/audit-profile.test.ts:36`: change `.toEqual(['math', 'physics', 'wenyan'])` to a **dynamic assertion against the registry's actual ids** — `expect(result.entries.map(e => e.id).sort()).toEqual(getDefaultSubjectRegistry().listIds().sort())`. (The `'passes for all built-in profiles'` test calls `auditProfiles()` with no args; after the fix, `auditProfiles()`'s *default* is still the hardcoded const — so to actually exercise the registry path, update this test to call `auditProfiles(getDefaultSubjectRegistry().listProfiles())` OR assert against `listIds()`. Use the `listIds()` dynamic assertion so a 4th subject auto-passes.)
- **Acceptance (machine)**: (a) `pnpm audit:profile` exits 0 and its output lists exactly the registry's `listIds()`; (b) `pnpm vitest run --config vitest.unit.config.ts scripts/audit-profile.test.ts` green; (c) a synthetic 4th profile registered in the registry would appear in the audit (proven by the dynamic assertion — no hardcoded 3-id list remains in `runCli`'s path). RL2 held.
...
- **Create** `src/core/schema/profile-studio.ts` (sits beside the existing `profile-decl.ts`; core layer, no IO):
  - `SubjectProfileDraftSchema` — a Zod schema for the draft-JSON wire format. **Distinct from `SubjectProfileSchema`**: the draft is the *editable input* (may omit/relax fields that publish assigns — notably `version` is optional on the draft per Q7, since publish/author assigns it). Mirrors `SubjectProfileSchema`'s field set but with draft-appropriate optionality. Inferred type `SubjectProfileDraft`.
  - `ProfileImpactReportSchema` — the compile-script output: `{ subject_id, valid: boolean, errors: string[], warnings: string[], diff: <field-level delta>, suggested_bump?: string }`. Inferred type `ProfileImpactReport`. The `diff` field is a structured field-level delta (added/removed/changed keys vs the current compiled profile), serializable to JSON for `--json`.
- **Export** from `src/core/schema/index.ts` (follow the existing barrel convention).
...
- **Create** `src/subjects/serialize.ts` (core-adjacent pure function, no IO — same shape as `src/server/export/csv.ts`; the file-write is the caller's, here in the `--write` path of Step 4):
  ...
  - **`version` is omitted from the serialized output** (Q4/Q5/Q7 — publish flow owns it; the author's existing `version` line is preserved separately or hand-filled). *Implementation note*: since the serializer emits the whole literal, "omit version" means the serialized literal does NOT include a `version` key — the `--write` path must preserve the author's existing `version` from the on-disk file (read-modify-write the version line) OR require the draft to carry it. **Decision: the serialized literal DOES include `version` from the input profile, but the CLI never *computes* or *bumps* it** — it passes through whatever `version` the validated draft carries (which the author hand-filled). This keeps the serializer a pure `profile → source` function while honoring Q7 "no auto-bump". *(This is the subtlest point — flagged for critic, §10.)*
...
- **Acceptance (machine)**: new `src/subjects/serialize.test.ts` (unit): **round-trip** — for each of the three current profiles, `serializeProfileToTs(profile)` produces source whose re-evaluated literal (parse via a dynamic import of a temp file, or eval-equivalent) deep-equals the original `profile` (strict equality on every field incl. Chinese strings). RL7: serializer touches no filesystem (assert it returns a string and writes nothing). `pnpm test:unit` green.

### Step 6 — ProfileCriticTask (the only MVP agent)

- **Modify** `src/ai/registry.ts`: add `ProfileCriticTask` entry — `needsToolCall:false`, `allowedTools:[]`, `budget:{...DEFAULT_BUDGET, maxIterations:1, timeout:60_000}`, `isMultimodal:false`, provider `xiaomi`/`mimo-v2.5-pro` (matches `SolutionGenerateTask`). The `systemPrompt` is the **runtime SoT** (subject-neutral, inline — Q3), instructing: review a draft `SubjectProfile` for overbroad taxonomy / missing capability / route ambiguity / prompt-template drift / fixture gap; output strict JSON `{ review_md, patches: [{field, suggestion, impact}], blocking: boolean }`; **propose only, never publish** (RL6 restated in the prompt).
...
- **Wire** the `--critic` flag in `scripts/compile-profile.ts` (Step 4) to call `runAgentTask('ProfileCriticTask', { draft }, { db, allowedTools: [], subjectProfile })` and emit the parsed review to stdout/`--json` (RL6 — no DB domain write, no file write; only the runner's own trace rows).
- **Acceptance (machine)**: (a) `pnpm typecheck` passes — proves the switch case + registry entry are consistent (omitting either fails `assertNever`/exhaustiveness); (b) new `src/ai/profile-critic.test.ts` (unit) asserts the registry entry shape (`allowedTools:[]`, `needsToolCall:false`, `maxIterations:1`) and that `getTaskSystemPrompt('ProfileCriticTask', anyProfile)` returns the **same** string for two different profiles (proves subject-neutral pass-through, Q3); (c) the `--critic` path, with the runner mocked, writes the review to stdout and makes **zero** writes to `profile.ts` and zero domain-row inserts (RL6 — assert via a spy on the serializer + a spy that no domain write fn is called). `pnpm test:unit` green.
...
- **Create** `src/ui/admin/subjects.tsx` — RSC surface: reads `getDefaultSubjectRegistry().listProfiles()` directly (Q6), maps each to the **slim subset** `{ id, displayName, version, notation: renderConfig.notation, capabilityCount: judgeCapabilities.length }`, renders `<main className="page wide">` + `PageHeader` + a `Card`/table of subjects (existing primitives; no full blob — R11). **Comment block at the top of the surface**: read-only; any future write MUST go through `/api/admin/*` (inherits `x-internal-token` gate); **no page Server Action** (RL5). The comment must NOT imply fields are immutable — it states "no write *entry point* yet (scheduled), not a policy that fields are fixed" (RL4).
...
- **Style authority (explicitly declared, since no loom doc applies)**: the existing `(admin)` page convention is the sole style authority for this page:
  ...
  - **Page shell**: `<main className="page wide">` + `<PageHeader …>` — verbatim from `src/ui/admin/observability.tsx:226-227` (`AdminRunsSurface` etc.).
...
- **Component type = RSC page** (React Server Component, server-rendered, read-only). Mounted under the existing `(admin)` route group → inherits `TokenGate` + `.app-shell` from `(admin)/layout.tsx`. **Not** a drawer, modal, or client component. No client interactivity beyond the layout's existing nav (the surface is a static read-only table).
...
1. **Serializer `version` handling (Step 5 / §3.4)** — the subtlest correctness point. The ruling is "serializer passes through the validated draft's `version` but never computes/bumps it" (reconciling Q4/Q5 "don't write version" with the practical need for the emitted literal to be a valid `SubjectProfile` that round-trips — and `version` is a required non-empty field in `SubjectProfileSchema:40`). An alternative reading of Q4/Q5 is "the serialized literal omits the `version` key entirely and the `--write` path read-modify-writes only the non-version fields, preserving the on-disk `version` line." **The plan picks pass-through (literal includes the input version, CLI never bumps)** because (a) it keeps the serializer a pure total function `SubjectProfile → string`, (b) a literal missing `version` would not type-check against `SubjectProfile` and would fail `validateProfile` (RL7 self-defeat), and (c) "no auto-bump" (Q7) is honored as long as the CLI never *mutates* the version. **Critic should pressure-test**: is "serializer emits version, CLI never bumps it" the right reconciliation, or does the user's intent require a version-line-preserving read-modify-write on `--write`? This determines whether Step 5's round-trip test asserts version-included or version-excluded equality.

2. **ProfileCriticTask DB-trace vs spec §4 "no DB" (Step 6 / §3.1, Map C1)** — the orchestrator ruling (Critic via CLI with **real db** to preserve the ai-run trace) directly contradicts the spec §4 literal phrase "single-shot 无工具无 DB". The plan resolves this by reading "no DB" as "no *domain/profile* DB writes" (RL6 proposal-only) while keeping the runner's own `task_run_log` + `cost_ledger` trace rows (evidence-first, per the project's AI-traceability principle). **Critic should pressure-test**: is preserving the ai-run trace the right call, or does "no DB" mean the Critic must use a no-log runner variant (Map C1 option B) so the CLI needs no `DATABASE_URL` at all? This determines whether the `--critic` env fence (Step 4) is necessary, and whether the acceptance asserts trace-rows-written (current plan) or zero-DB-touch.
