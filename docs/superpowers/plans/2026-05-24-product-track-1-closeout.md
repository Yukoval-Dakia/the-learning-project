...
- **[YUK-66](https://linear.app/yukoval-studios/issue/YUK-66) Teaching chat ask_check 落 question artifact** —— Phase 2C MVP 故意 defer，现 teaching chat `ask_check` 只作 chat 消息存（`event(experimental:teach_message, turn_kind='ask_check')`），不写 `question` 表 / 不接 judge / 不进 FSRS / 不喂 [YUK-17](https://linear.app/yukoval-studios/issue/YUK-17) variant chain。Phase 3 scope，跟 [YUK-53](https://linear.app/yukoval-studios/issue/YUK-53) Note inline check 共享 EmbeddedCheck artifact pattern。Discovered during [YUK-47](https://linear.app/yukoval-studios/issue/YUK-47) E2E
