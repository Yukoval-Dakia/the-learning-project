...
| worker copilotTools half-wired = 硬前置、reach+endurance 双重承重 | ✅ 成立但**非功能阻断**：worker 跑 AI job 经 `buildMcpServerFromRegistry` 的幂等 `registerCoreTools()` 拿 CORE_TOOLS 超集（41⊋26），功能不缺工具（YUK-362 已记） | **不在 W1**：`registerCapabilityCopilotTools` 接线只在 CORE_TOOLS *退役*（post-M5）时才需要，YUK-362「不做」节 deferred；新 copilot_run handler 走 buildMcpServerFromRegistry 自动有全集 |
...
#47 reach 结构型 typed apply + run_task dispatcher 契约 + 单 applier choke point ─┐
   后端 · 独立                                                                    │
YUK-363 cascade CTE 遍历器（Wave 0 引入） ─────────────────────────────────────────┤─(收束)→ #47 checkpoint（per-utterance PR）
                                                                                  │    gated 在「单 applier」+「cascade」+
YUK-363 Wave 1/2/3 全 projection fold（copilot 写面: knowledge/edge/artifact/ ─────┘    **copilot 全写面已 event-sourced」三就位
   learning_item/question_block.structured）                                            （否则撤不动命令式写——Codex P2）

#47 endurance durable run（新 copilot_run handler 照 quiz_gen + 复用 job_events）
   后端 · 独立 · ~90% 现成 · 零前置 ── run card UI / summarize-continue 可拆后续 lane
```
...
3. **copilot 全写面 projection 已 event-sourced**（YUK-363 Wave 1/2/3）= checkpoint 能撤的上限 = 已 fold 化的写面；copilot 的 artifact/learning_item/structured 写不 event-source 化，级联 revert 撤不动 → **所有结构 projection 改造必须先于 checkpoint**（Codex P2，原误排 W6 只依赖 W3+W4）。
4. **π_i 持久化**（#46 选题引擎 L1）= #45 active-PPI rectifier 必需 → #46 必须先于 #45（且 #45 还 gated 在 owner 真实作答数据攒够，阶段③）。

## 3. 波次序（串行 gate，「一次盯一条」；owner「∥」= 跨 epic 交错 lane 非并发开多个重 workflow）

| Wave | lane | epic | 依赖 | UI? | 备注 |
|---|---|---|---|---|---|
| **W1（首发）** | copilot endurance durable run（后端） | #47 | 无（main 现成 job_events；worker 工具/causedByEventId 已就位） | 否 | 桥同步→异步；新 copilot_run queue+handler 照 quiz_gen（走 buildMcpServerFromRegistry 自动有全集工具） |
...
| **W6** | YUK-363 Wave 2+3（goal/mistake/learning_item + artifact/question_block.structured 真 fold，合 YUK-358） | YUK-363 | W5 | 否 | **必须在 checkpoint 前**——copilot 的 author_artifact/update_artifact/propose_question_edit/learning_item 生命周期写面都落这些表，没 event-source 化它们 checkpoint 的级联 revert 撤不动（Codex P2） |
| **W7** | checkpoint（per-utterance PR + 级联 revert 编排） | #47 | W3 + W4 + W5 + **W6**（单 applier + 全 cascade + **copilot 全写面 projection 已 event-sourced**） | 是（run card / PR diff 审）→ 先后端契约，UI 走 claude design handoff | 三腿收束于一处；放在所有结构 projection 改造之后 |
| **W8+** | #47 summarize-continue · endurance run card UI · #45 π_i 去偏（阶段③，gated owner 数据） · 长尾 | 混 | 各自前置 | 部分 | 按需求拉起 |

**为什么 W1 选 endurance**：owner 点名「启动 #47」；grounding 证 ~90% 现成 + 纯后端 + 加性（不重构高风险 reach 面）+ 照 quiz_gen 既有模式 + 零前置（worker 工具超集 + causedByEventId 透传 + job_events 基建都已就位）→ 风险最低、立刻动得了。reach 重构（W3，触及五处手接线 + rename）押后到契约更稳。

**为什么 W2（#46）紧跟**：product 脊柱、独立、直接服务北极星「留存 + 代理信任」、解锁 #45。owner「∥」精神 = #47（W1/W3/W7）与 YUK-363（W4/W5/W6）交错推进，#46 作独立产品线插入。**checkpoint（W7）落在所有结构 projection 改造（W4/W5/W6）之后**——它能撤的上限 = 已 event-sourced 的写面（Codex P2 修正：原 W6 只依赖 W3+W4，会让 checkpoint 撤不动 artifact/learning_item/structured 的命令式写）。
...
- **零前置确认**（grounding）：worker 工具超集（buildMcpServerFromRegistry 幂等 registerCoreTools）+ causedByEventId 透传 + job_events 基建均已就位；worker manifest 接线属 CORE_TOOLS 退役（post-M5, YUK-362「不做」），**不在 W1**。
