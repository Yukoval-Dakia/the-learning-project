...
| 文件 | 动作 | 说明 |
|---|---|---|
| `app/(app)/inbox/proposal-shared.tsx` | **创建** | `BlockMergeProposalCard`、`kindLabel` / `ProposalStatusRow` 等 leaf helper、以及共享展示用类型（`ProposalKind` / `ProposalTarget` 等）的落地处。**实施修正**：Next.js 不允许 route `page.tsx` 导出任意命名符号（"not a valid Page export field"，且该校验只在 `pnpm build` 跑——tsc / biome / vitest 全放行），故可被 unit import 的可测面被抽到这个同目录非保留模块里；`page.tsx` 从此导入。详见文件顶部注释。 |
| `app/(app)/inbox/page.tsx` | 修改 | 在卡片 dispatch 里为 `block_merge` 分流（渲染从 `proposal-shared.tsx` 导入的 `BlockMergeProposalCard`）；复用 `acceptMutation` 并处理 `stale` 结果。本地 `ProposalKind` union / `KIND_LABELS` / `kindTone` 的 `block_merge` 等漂移修正落在 `proposal-shared.tsx`（page.tsx 改为消费它）。 |
| `app/(app)/inbox/inbox.test.tsx`（若不存在则**创建**） | 创建/修改 | 组件级单测（unit 分区）：`block_merge` 卡片接受按钮 enabled、点击触发 accept、stale 反馈。组件从 `proposal-shared.tsx` import（非 route `page.tsx`），可在无 DB 下渲染归 unit；否则降级为后端已覆盖 + 手测（见 §4 决策）。 |
...
- **组件类型声明**：`block_merge` 走**新 card 组件**（presentational card，非 drawer/modal/page），与既有 `EdgeProposalCard` / `NodeProposalCard` 同构。**实施修正**：因 Next.js route `page.tsx` 不接受任意命名导出（详见文件清单中 `proposal-shared.tsx` 行），卡片实际落在同目录 sibling 模块 `app/(app)/inbox/proposal-shared.tsx`，由 `page.tsx` import，而非内联在 route 文件里。
...
> **实施修正（落地实况）**：以下步骤把 `BlockMergeProposalCard`、`ProposalKind` union / `KIND_LABELS` / `kindTone` / `kindLabel` 描述为直接改在 `page.tsx` 内。实际落地时因 Next.js 不允许 route `page.tsx` 导出任意命名符号（"not a valid Page export field"，只在 `pnpm build` 才暴露），这些可测的展示面被抽到同目录 sibling 模块 **`app/(app)/inbox/proposal-shared.tsx`**（见 §2 文件清单与该文件顶部注释），`page.tsx` 改为从它 import。下文凡提到「在 `page.tsx` 新建/补」的类型、标签、card 组件，实际落点均为 `proposal-shared.tsx`；`page.tsx` 仅保留 dispatch 接线与 mutation。
