...
| P3 | Legacy overflow-bbox rows clamped (structured/figure bbox within 0-1) | question_block | ⏳ §9.3 data-fix sweep — the genesis backfill fails loud on these; clamp them (the C1-δ flat8ToBBox fix prevents NEW ones). Run the backfill on a prod-clone first to surface the list (now batched: one error lists every bad id). |
