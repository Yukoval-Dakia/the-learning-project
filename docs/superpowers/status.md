...
**最后更新**：2026-06-06（**OC-5 复查面 + VisionTab prefill ✅ shipped**：YUK-164 #2/#3 落 main——GET `/api/ingestion` session list + `mistake_draft` surfacing（slice 1）+ AutoEnrolledPanel 5th tab + `src/ui/lib/auto-enroll.ts`（slice 2）+ VisionTab AI 预填 + "AI 预填，可改" badge（slice 3）；PR 待开；flag #4（`WORKFLOW_JUDGE_AUTO_ENROLL_ENABLED` flip）留 YUK-164 #4 未翻。详见下方「OC-5 ✅」）；同日（**Strategy D S3 ✅ 三片 shipped**：图片素材可达性 YUK-227/YUK-228 收官——Slice A VLM 图题空间匹配 + F4 spatial 消费、Slice B Note 族 skill 三科、Slice C image_candidate 提案 + accept 兑现 VLM 抽图（守 ADR-0002 付费授权语义），PR #323-#325，详见下方「Strategy D 章 · S3」）；2026-06-06 同日（**Strategy D S2 ✅ 全五片 shipped**：题源扩展 YUK-216 收官——provenance/tier 地基 + SourcingTask 检索线 + material 素材线 + **标准 Agent Skills 规范双轨（owner「代码只留任务描述」哲学第一例生产接线）** + 消费端 tier 偏好与四线找题次序，PR #312-#320，详见下方「Strategy D 章」）；2026-06-05 晚（**Strategy D S1 ✅**：ingest→practice 桥接 + judge 手写照片穿透落 main（PR #309 rebase-merge））；2026-06-05（**U 序列 drive ✅ DONE**：U0-U8 全部落 main（U7 末棒 PR #307），三份 codex 设计统一落地完成，详见下方「U 序列 drive ✅」；本条随 U7 post-merge 收官）；2026-06-01（**数据激活 drive ✅ DONE**：Station 1 合成数据 + 2A brief writer + 2B goal cron + 3 端到端验证（8 slice 全绿，零集成 bug）→ Layer-8 端到端可观测；详见上方「数据激活 drive ✅」+ closeout。下一站 Strategy D 真实 ingestion，开工前 surface 用户。）；2026-05-31（**P5 open-questions chain ✅ 全 7 阶段 merge → origin `82ba1df1`**；详见「P5 chain ✅」+ closeout。）；2026-05-30（**Wave 7 ✅ → origin `28dd48d6`**（P5 反链/cross_link/hub auto-sync YUK-95 + T-KG cytoscape 诊断图谱 YUK-142 + fix-pass + closeout）；**Wave 8 ✅ 实现完成**（末波 = P6 read-view+节点页 YUK-96 + P7 tests YUK-97 + P2-polish slash/drag-drop YUK-150 + 两轮用户 review-fix + T-PD 收尾 + v1 closeout，全 7 lane 各独立 opus reviewer + 完整 wave-gate 绿：145 test files / 1153 tests + 1 todo / migration 11 / build 55 pages），**Wave 8 已 merge 到 main**（2026-05-29，PR #193 rebase-merge，保留 per-commit Closes），其上叠了 scenario B 第一批。上一波 Wave 6 `a419b2e6`。）
**当前 Phase**：**Strategy D 章（真实 ingestion + capture 飞轮，2026-06-05 开工）** —— kickoff 简报（6-agent Map）→ **S1 桥接 ✅** → **S2 题源扩展 ✅**（YUK-216）→ **S3 图片素材可达性 ✅**（YUK-227/228，2026-06-06 收官）。下一站候选：日常使用验证期（owner 真实 ingest + 做卷积累 observe trail 与真题库——S1-S3 机器齐备，真实数据是当前最大缺口）/ **OC-5 复查面 ✅ shipped**（YUK-164 #2/#3，PR 待开；flag #4 未翻）/ UI wave（YUK-229 inbox accept + tier badges 等积压）。

**OC-5 复查面 + VisionTab prefill ✅ shipped（2026-06-06，PR 待开，Refs YUK-164）**：auto-enroll 观察态复查面 + VisionTab AI 预填（YUK-164 #2/#3）三 slice 落分支：
- **Slice 1 后端读形状**：新增 `GET /api/ingestion?limit=N` session list（newest-first，含 `observation_count` / `auto_enrolled_count` / `block_count`）；`toAutoEnrollObservation` 扩展 `mistake_draft` 字段（tolerant projection：`wrong_answer` / `difficulty` / `cause.{primary_category, analysis_md}`，历史缺字段静默 null 不整体丢弃）；wire-shape 注释更新（blocks/route.ts:23-25 旧注释修正）；DB 测试扩展（GET list + `mistake_draft` round-trip）。
- **Slice 2 AutoEnrolledPanel + 共享 lib**：新增 `src/ui/lib/auto-enroll.ts`（`AutoEnrollObservation` 类型 + `formatConfidence` + 纯函数 `seedBlockForm`）；新增 `AutoEnrolledPanel.tsx`（5th tab `auto_enrolled`，自含 TanStack Query，Stateful 五态，observe-only banner 从 `blocksQ.data` 派生，per-row revert inline 二步确认）；`record/page.tsx` 加第五 tab；unit 测试（纯函数直接断言 + `renderToString` 静态 HTML）。
- **Slice 3 VisionTab prefill**：`BlockRow` 增 `auto_enroll_observation` 字段 + `status` union 扩 `auto_enrolled`；seed effect 调 `seedBlockForm`，有观察时预填 `knowledge_ids` / `difficulty` / `cause_primary` / `cause_notes`（映射自 `cause.analysis_md`，非不存在的 `user_notes`）；`wrong_answer` 作 read-only AI 提示行；"AI 预填，可改" `Badge tone="info"` badge；`auto-enroll.test.ts` 扩 `seedBlockForm` 用例；新增 `VisionTab.test.tsx` 静态 HTML（badge 有无断言）。
- **deferred/留后**：flag flip `WORKFLOW_JUDGE_AUTO_ENROLL_ENABLED` → YUK-164 #4 未翻；视觉 pass → redraw wave2 负责（plan 明确"minimal-functional，视觉是 redraw 的工作"）；revert 路径真实点击验证（flag OFF 生产无 `auto_enrolled` 行，service DB 测试已覆盖）。

**Strategy D 章 · S3 ✅ shipped（2026-06-06，PR #323-#325）**：图片素材可达性（YUK-227 主线 + YUK-228 副线；plan Cross-统合 + owner F1-F4 裁决）三 slice 落 main：
- **Slice A 图题空间匹配**（#324）：StructureTask 单次输出承载 figure 归属（`StructureNode.figure_ids` + `figureAssignments` 映射，F3 默认=零新增 task/付费点）+ `assignFiguresFromVlm`（VLM high-confidence 优先 / 未覆盖图几何兜底不丢图 / 幻觉 id 守卫 / 同 target 重复视为无害冗余）+ page_spans 真实化（腾讯回落强制 placeholder）+ **F4 spatial 消费**：path-B 页信号参与 block 匹配，全 placeholder 退 semantic-only 零回归。slice-2b 遗留「多图全挂第一题」失败模式关闭。
- **Slice B Note 族 skill**（#323，YUK-228）：三科 `note-<subjectId>/SKILL.md`（subject 级激活键裁决——artifact_type 是 task 内 type switch，不构成分目录维度）+ `resolveNoteSkill` never-throws + NoteGenerate/Verify/Refine 三 handler 接线 + verify prompt 真删薄（四维/阈值 fallback 保留，skill 缺位降级合格）。S2-4 基建第二例消费，runner 零改动。
- **Slice C 图片源 sourcing**（#325，F1 拍板 (a) 提案+accept）：`image_candidate` 提案枚举纵切（payload variant + AC-2 守护×2 + corrective 映射 + accept dispatch 四处强制同步；零 DB migration 实证——proposal kind 走 event JSONB）+ accept 兑现链（下载→R2 `source_asset`→VisionExtract manual_rescue_only→tier-2 草稿→source_verify）。**ADR-0002 守住：accept 是全链唯一 VLM 触发点**，`writeCostLedger(sourcing_image_extract)` per-accept 留痕（零值关联行防双计，真实花费在串联的 VisionExtractTask 行）。
- **流程账**：Slice A/B 并行双 lane（worktree 隔离）各自独立 review+bot 收敛；Slice C 三 bot（Codex/CodeRabbit/OCR）两轮 28 条去重 19 项合批修复——SSRF 重定向逐跳重验 / 成本双计（OCR 要非零×Codex 要不双计×plan 要留痕三约束求交→零值关联行）/ accept 窗口否决竞态 409 / MIME allowlist / requested_kind 透传（question-kind.ts 单一权威复用）/ IPv6 字面量误杀。验证轮教训：**声称的「生产路径」修复必核 default seam 真转发**（defaultRunTaskFn 丢 task_run_id 半闭合实证）。owner 砍 round-3 直接 merge。
- **deferred/留后**：YUK-229 inbox image_candidate accept UI（平台级 YUK-44 同根：GenericProposalCard accept 全 kind disabled）· YUK-230 多模态 grounding gate（extract 单源自证局限，已标 `single_source_grounding`）· 409 否决后孤儿 asset/R2 对象（P3 信息性，单用户无害）· OCR CI per-run 评论（PR #326 等 merge）。

**Strategy D 章 · S2 ✅ shipped（2026-06-05 → 06-06，PR #312-#320）**：题源扩展（YUK-216，spec 五轮 brainstorm + 外审五修正 + Agent Skills 轨 1 定稿）全五片落 main：
- **S2-1 地基**（#312）：§2.1 provenance 合约四条 + `deriveSourceTier`（provenance 优先推导）+ per-tier 验证门 + **solve-check**（独立 solver 真解题，OF-4 拍板语义）。
- **S2-2 检索线**（#313）：SourcingTask 按需检索 + 自动入库 draft + tier-2 验证（extract 必锚 / OF-2 降权落标）；与 #315 的语义合并缝隙（OF-2 fixture × extract 必填）实证后由 S2-4/5b 等价修复治愈。
- **S2-3 素材线**（#314）：`material_grounded` + source_document 持久化 + **素材嵌入题面（学习者可见）** + 真 grounding verdict 校验——阅读题真原文判据兑现。
- **S2-4 规范双轨**（#319）：首批三 **标准 Agent SKILL.md**（wenyan 翻译/阅读 + math 计算）+ runner 接线（YUK-217 spike 修正方案：CONFIG_DIR/skills 预置 + settingSources OMITTED + `Options.skills` 键控；**skills omitted=全启用语义反转**实证后改显式 `[]` 默认禁用）+ Docker skill 资产 + profile sourceWhitelist 四候选（OF-1 调研）+ few-shot 检索器（轨 2）。
- **S2-5**（#315/#320）：5a 读模型 source/tier + due-list 穿透（B1 回归守护）+ OF-2 降权排序；5b 四线找题次序编排（B+ 调用次序 + B2 同步/异步边界 + **kind 三词表统一规范化层**——core 枚举/profile 键/skill 键错配实证后单一权威映射收口）。
- **流程账**：5b 谱系 5 轮 22 实质点收敛（owner 批准追修先例）；三教训：kind 词表错配、语义合并缝隙（两 PR 各自绿但组合红——gate 不覆盖未合并组合）、SDK skills 语义反转（spike 验证了「怎么生效」没验证「不传是否无影响」）。
- **deferred/留后**：白名单正式值随使用调优（profile 注释含 owner 待确认项 gaokao.zxxk.com）；图片型源 Vision 抽取（YUK-223 注记）；YUK-221 block 真序键。

**Strategy D 章 · S1 ✅ shipped（2026-06-05，PR #309）**：
- **YUK-214 ingest→practice 桥接**：`POST /api/ingestion/[id]/make-paper`（显式形态 b，owner 拍板）把 imported session 整卷打包 `tool_quiz` artifact（新第四 paper 源 `ingestion_paper`），`/practice` 消费端零改动；幂等按 session（集合不符 409 + 归一化比较）；组卷序对齐 `/blocks` 屏上序（真 block 序键不存在——实证发现，YUK-221 留单）。**真实数据飞轮 ingest→做题→FSRS 闭环打通**。
- **YUK-215 judge 手写照片穿透**：两条提交路径把 `answer_image_refs` 真传进 judge；photo-only 仅对 image-consuming 路由（steps/multimodal_direct）放行判分，文本路由不判不污染 FSRS/统计（`unsupported_judge` 加性标志），supplied-result 同门控。附带：judge 路由解析提取 leaf 模块（webpack chunk 环根因，实证二分定位——连 type-only 回边都在环里）。
- **流程**：plan→Critic→Cross-统合（0 BLOCKER）→ impl r1 即 SHIP → 双 bot 4 轮 18 findings 全处置（15 修 + 2 有据驳回 + 1 实证降级留单）→ **owner 裁停 round-5**（收敛判据精化：实质轮加验证轮、owner 可随时裁停）→ 全量 gate 绿 merge。
- **同日产出**：YUK-216 题源扩展 spec（五轮 brainstorm + 外审五修正 + Agent Skills 轨 1 定稿）· SDK 对齐大审计（`docs/audit/2026-06-05-agent-sdk-alignment.md`，A1/B3/C6/D2，主轴 = prompt fragments→SKILL.md）· 新 issue YUK-216~221 · **改判**：外网访问（tunnel/反代）出项目范围（TUNNEL_TOKEN rider 撤）。
- deferred backlog 维持：legacy chat cut-over（YUK-213）/ YUK-209/210/211/212。

**U 序列 drive ✅ shipped（2026-06-04 → 2026-06-05）**：
...
- **U6 ✅**（PR #305，plan：[`plans/2026-06-05-u6-chat-merge.md`](plans/2026-06-05-u6-chat-merge.md)）：teaching/solve 合并进 Copilot skills（AF S4）——`skill_context` 选择器（S2b active_ref 前向种子）+ 服务层组合 TeachingTurnTask（**单 session**，Critic 抓掉双 session 的 idle-cron→accept-chip-409 CRITICAL 链）+ resolver/turn_kind 配对锚 + replay 全持久化（skill_turn/skill_context/锚点，刷新不丢卡不丢 chip 不断教学连续性）。**共享 LLM 输出方差加固**（控制字符状态机转义 + choices_md coercion 降级，legacy 同享——视觉环真实流量抓出的两类 500）。R1-R7 红线逐条核证；legacy teaching 全链路按 R3 并行保留。双 bot 2 轮 11 findings 全修按收敛判据收口。
- **U7 ✅**（PR #307，plan：[`plans/2026-06-05-u7-studio-mvp.md`](plans/2026-06-05-u7-studio-mvp.md)，末棒）：Profile Studio MVP 七项全量——D6 readiness 确认 + **YUK-206**（audit:profile 走 SubjectRegistry，合成第 4 学科测试守护）+ SubjectProfileDraft/ProfileImpactReport Zod + compile CLI（**结构性不可绕** validate-before-write gate + 路径注入防护 + 单一 JSON 输出）+ serializeProfileToTs（toStrictEqual round-trip 双 fixture）+ ProfileCriticTask（live 实测：review_md + 7 patch 提议 + trace 落账，proposal-only）+ read-only /admin/subjects（RSC 直读 registry）。YUK-172 两半显式 OUT；DEFER 项零偷做。双 bot 2 轮 11 findings 全修按收敛判据收口。
- **流程升级（用户定 + 实战教训）**：phase workflow = Map→Plan→Plan-Critic→**Cross-统合**→Implement→独立Review→Fix→集成gate(orchestrator亲跑)→视觉环→PR→双bot(召唤+窗口)→merge；视觉环（playwright+原生视觉）启用；verify-by-reconstruction；post-merge smoke；**双 bot 收敛判据**（P1 清零 + 末轮全 P2 边角即收口）。
...
- **T-OC / YUK-145**：slice1 generalized capture（PR #197，**ADR-0024**）/ slice2 VLM StructureTask + multi-page YUK-144 rootfix（PR #199）/ slice3 TaggingTask+WorkflowJudge+flag-gated-OFF auto-enroll（PR #202，**ADR-0026**）。DEFERRED：slice-2b figure-matching YUK-163；slice-3b auto-enroll wiring YUK-164 #1（观察态接线）。**OC-5 review surface + VisionTab prefill YUK-164 #2/#3 ✅ shipped**（PR 待开，2026-06-06）；flag flip YUK-164 #4 留后。
...
✅  U6    三 chat 合并             teaching/solve → Copilot skills（AF S4，单 session + skill_context；PR #305）
✅  U7    Studio MVP               compile CLI + serializer + ProfileCriticTask + /admin/subjects（PR #307）
...
```
✅  M5    Copilot 收口 + 拆除      copilot+observability 两 capability 包 + kernel `copilotTools` 贡献制（D14 单人格）+ 旧栈采石场单 commit 拆除（Next.js 15 App Router 全退场，192 files −23601；gate 选项 b：双进程 + presence 进 PG 表 editing_presence，无 Redis；Hono :8787 + Vite SPA :5173 + pg-boss worker 三进程拓扑；D19 路由壳文件仪式消失）（PR #394，2026-06-13）
```
