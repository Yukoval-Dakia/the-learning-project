...
> 见 [架构基础](../architecture.md) 了解 `Question` / `Answer` / `Judgment` / `UserAppeal` schema 和相关 Task 注册。

横跨多模块的子系统：每日 quiz / final quiz / embedded check 都是 Question 的实例，统一引擎、统一判定、统一申诉路径。
...
Quiz 子系统只关心一道题的「问 → 答 → 判」生命周期，结果通过事件喂给其他模块。
...
每种 judge_kind 对应一个独立 Task（见 § 8）。Question 有默认 judge_kind，可在题目级用 `judge_kind_override` 强制。
...
6. 副作用：
   ...
   - 若反向变化：（不太可能，理论上 ai_flexible 不会比初判更严，但保险起见处理）创建 mistake
   - 重新喂 mastery
...
为防恶意烧 ai_flexible 预算：**同一 Judgment 最多被申诉 1 次**（可配置）。已申诉过的 Judgment 不再显示申诉按钮。

---

## 9. Schema 详细

完整 schema 见 [架构基础 § 五](../architecture.md#五数据模型骨架)。这里展开关键字段。
...
Question
  ...
  prompt_md                     # 题面
  reference_md                  # 参考答案
  rubric_json?                  # rubric 类用：{criteria: [{name, weight, descriptor}]}
  judge_kind_override?          # 强制特定判定方式
...
Answer
  ...
  content_md                    # 文字答案；image 路径下走 pipeline 时由 vision_extracted 填充
  image_refs[]                  # 原始图片 path / blob refs
  vision_extracted?             # pipeline 路径下 vision 转文字结果
  tags?: ['handwritten', 'partial_visible', ...]
  submitted_at
...
Judgment                        # 不可变；同 answer_id 可有多条
  ...
  score: 0~1
  feedback_md
  evidence_json                 # AI 推理痕迹（ai_flexible 路径必填详细 CoT）
  is_flexible_fallback: bool
  triggered_by?: initial | borderline | appeal | force
  prior_judgment_id?            # 兜底 / appeal 重判时指向上一次
  judged_by: {task, provider, model, version}
  judged_at
  is_effective: bool            # 同 answer_id 中最新的为 true，旧的为 false

UserAppeal
  id, judgment_id
  reason: text?
  appealed_at
  resolved_judgment_id?         # ai_flexible 重判后写入
```
...
| --- | --- | --- | --- |
| `QuizGenTask` | Sonnet (+ batch 可选) | embedded check / daily / final / 用户主动 | 参数：`{source, count, difficulty, kind_distribution, knowledge_ids}` |
...
| 模块 | 衔接方式 |
| --- | --- |
| [mistakes](mistakes.md) | `Judgment.verdict in [partial, incorrect]` 时创建 Mistake（保留 Question + 用户错答 + 推理痕迹）。Appeal 翻盘时撤销 Mistake (标 misjudged) |
...
| [notes](notes.md) | Embedded check = atomic.section.kind=`check`，section 含 question_id 引用；做对喂 mastery，做错入错题 |
| [lanes](lanes.md) | 每日 quiz = `Question.source=daily`，由 dreaming 夜间 batch 生产；接受率 < 30% 触发砍信号 |
...
```
User → POST /answer { content, question_id }
  ...
  │    create Mistake (link question_id, answer_id, judgment_id)
...
partial 也算"答错入错题本"，但 mistake.cause 标 partial，复习时可酌情处理。
...
5 条都 working = quiz 子系统 validated。**Phase 1 不上 multimodal_direct 和 steps**——文言文场景没图、没计算。

---

## 15. Phase 路线

### Phase 1（含 quiz 子系统骨架）

- [ ] Schema: Question / Answer / Judgment / UserAppeal
- [ ] QuizGenTask（参数化生产，含 embedded / daily 模式）
- [ ] JudgeRouter（路由逻辑）
...
- 同一题多次答（学习中反复练）：每次新建 Answer，旧 Answer 保留（决策：保留全部历史）
