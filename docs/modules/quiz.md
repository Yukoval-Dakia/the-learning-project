...
完整 schema 见 [架构基础 § 六](../architecture.md#六数据模型骨架)。这里展开 quiz 子系统的关键字段。

### 9.1 tool_quiz Artifact
...
Artifact
  id, type=tool_quiz
  title                           # 如 "2026-05-08 每日 quiz"
  knowledge_id?                   # 主关联知识点（如有）
  parent_artifact_id?             # 通常 null（standalone）；若需挂 LearningItem 用 source_ref
  intent_source: declared | from_mistake | from_dream
  source: daily | final | dreaming | manual | mistake_variant
  source_ref?                     # learning_item_id (final) / proposal_id (dreaming)
  tool_kind: 'quiz'
  tool_state: {
    question_ids: [string]        # 引用 Question 表
    session_meta?: {              # 可选：保存 session 状态
      time_limit_seconds?,
      shuffle?: bool,
      ...
    }
  }
  generation_status, generated_by, history, archived_at?
  updated_at, version
```

### 9.2 Question / Answer / Judgment / UserAppeal

```
Question                          # 题目本身（独立表，可被多个 tool_quiz Artifact 引用）
  id
...
### 9.3 Embedded check vs Standalone tool_quiz
...
| 生命周期 | 跟 note section 1:1 | 独立（每日新生 / 用户保存） |
| 可重做 | 可重做（生 Answer + Judgment） | 可重做 |
| 可跨 LearningItem 复用 | 否（绑死 note） | 是（用户可用同一模拟卷反复刷） |
| Artifact archived | 跟随 note | 独立归档 |

设计取舍：embedded check 不独立成 Artifact 行，避免大量小 row（每个 atomic 都有 check section）；standalone tool_quiz 是独立资源，需要 archived / 引用 / 跨 session 复用。
...
| --- | --- | --- | --- |
| `QuizGenTask` | Sonnet (+ batch 可选) | embedded check / daily / final / 用户主动 | 参数：`{source, count, difficulty, kind_distribution, knowledge_ids, target}` 输出 `Question[]`。`target='embedded'` 时 caller 把 ids 填到 note section；`target='standalone'` 时 caller 创建 tool_quiz Artifact |
...
| [lanes](lanes.md) | 每日 quiz / dreaming 题目推荐都生成 standalone tool_quiz Artifact |
...
partial 也算"答错入错题本"，但 mistake.cause 标 `partial: true`，复习时可酌情处理。
