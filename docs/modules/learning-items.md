...
| AI 主动提议完成（`event(action='propose')` payload `kind='completion'`） | ❌ | Phase 2 dreaming worker；走 [`src/core/schema/proposal.ts`](../../src/core/schema/proposal.ts) AiProposal union — `completion` / `relearn` discriminants 已在 schema |
...
```
event(action='propose', subject_kind='learning_item', actor_kind='agent')
  payload: AiProposal {
    kind: 'relearn',
    target: { subject_kind: 'learning_item', subject_id },
...
| AI 主动提议 | 满足任一：mastery>0.8 持续 14 天 ∨ 关联 check 全过 ∨ 该知识点 7 天错 0 | 走 `event(action='propose')` + AiProposal `kind='completion'`，详见 § 3.2 |
| Quiz 通过 | 用户选择走严格路径 | 出题 + 评分（详见 [`quiz.md`](quiz.md)）；通过 = 所有关联 `attempt` event 对应的 `judge` event verdict==correct，base mastery 硬跳升到 ≥ 0.7（Judgment 表已 DROPped per ADR-0006 v2） |
...
进 dreaming 待审核区（`GET /api/proposals?status=pending`）。用户次日打开 app 看到通知 → 一键 approve / dismiss：
- approve → 创建 `completion_evidence(path=ai_propose)` + `learning_item.status='done'`（`POST /api/proposals/[id]/accept`）
- dismiss → 该 LearningItem 短期内（默认 7 天）AI 不再提议同一项（`POST /api/proposals/[id]/dismiss`，冷却由 `proposal_signals` 跟踪）
...
| AI 主动提议完成 | dreaming → learning-items | `event(action='propose')` + AiProposal `kind='completion'` |
| AI 主动提议重学 | dreaming → learning-items | `event(action='propose')` + AiProposal `kind='relearn'` |
...
- AI 主动提议完成走 `event(action='propose')` + AiProposal `kind='completion'`（schema: `src/core/schema/proposal.ts`）
