...
- 算 LearningItem done 信号之一（见 [`learning-items.md`](learning-items.md)）
- 直接喂 base mastery（见 [`progress.md`](progress.md)）
- 错的题入错题本（见 [`mistakes.md`](mistakes.md)），闭环回到错题管理
...
## 自检
<!-- embedded-check id="check-abc" -->
```

Embedded check 用 HTML 注释占位 + 渲染层替换：本工具里 → 交互组件；Obsidian/VS Code 里 → 看不见的注释。Section ↔ markdown heading 一一对应，AI 更新某 section = 编辑该 heading 下的内容。
...
- 阅读 UX（hub↔atomic 导航 + 移动 / 桌面差异）
- 跨学科引用（数学 note 引物理 atomic）
- Note 版本演化的 UI（让用户看到「3 天前更新了 example」）
- 「我已经会一部分」快速跳过机制
