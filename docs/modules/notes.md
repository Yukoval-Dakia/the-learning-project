...
> 见 [架构基础](../architecture.md) 了解 `Artifact` schema。
> Embedded check 引擎详见 [`quiz.md`](quiz.md)。
...
每个入口都创建 LearningItem（见 [`learning-items.md`](learning-items.md)），并触发 `NoteGenerateTask`。
...
| `example` | 标准例子（1~3 个） |
| `pitfall` | 易错点 / 常见误解 |
| `check` | 自检 mini-quiz（1~3 题，引用 `Question.id`） |

`check` section 不内嵌题面，而是引用 `Question` 表里的题（见 [`quiz.md`](quiz.md)）。这样 embedded check 跟每日 quiz、final quiz 共用一套引擎。

**理由**：结构化才能做"针对你最近的错题更新这一节"。完全自由的 markdown 只能 regenerate 整篇。
...
`check` section 末尾的 mini-quiz 是 [`quiz.md`](quiz.md) 的 Question 实例（`source: embedded`）：
...
读 note 与做题不再分离，且共用 quiz 子系统的判定 / 申诉机制。
...
sections:
  ...
  - id: check
    kind: check
    question_ids: [q-001, q-002]   # 引用 Question 表
version: 3
...
- 阅读 UX（hub↔atomic 导航 + 移动 / 桌面差异） → **已定**：移动单栏线性，桌面双栏（侧栏 outline + 主体）
