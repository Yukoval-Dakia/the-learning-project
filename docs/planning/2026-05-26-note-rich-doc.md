...
| **P1 ADR 范围分担** | ADR-0020 定核心契约（anchor / cross_link / semantic_kind / archive / version 模型）；具体 TipTap PM node schema 在 P2 跑通后补 ADR-0021；P2 实现期间 schema 微调允许（每次更新 ADR） | C4.2.c |
...
- **ADR-0021（新，PM node schema）**：TipTap PM doc 具体 node 形态（P2 跑通后补）
...
| **P2 — TipTap 编辑器接入（16 pts）** | TipTap 集成 + 自定义 nodes（SemanticBlock / CrossLinkBlock / ArtifactRefBlock / CalloutBlock / **AutoLinksContainer**）+ slash command + inline mention + drag-drop + paste markdown + undo/redo + `<BlockTreeRenderer>` SSR mode + bundle 拆分 + 跑通后补 ADR-0021 | 5-6 周 |
...
| **P3 + P4 并行可能撞 schema** | P1 ADR-0020 钉死核心契约；P2 跑通后补 ADR-0021 锁 PM node schema；P3/P4 在 ADR-0021 后才可并行 |
