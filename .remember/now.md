...
- 遗留信号：jyeoo-rs 目录非 git repo（修复只以文件+二进制存在，立票中）；JYEOO_SPAWN_TIMEOUT_MS 默认 120s 对 grade 路线偏小（立票中）；matcher 接 live caller 后 axis C 复测（立票中）；复验在共享 DB 落 4 条真实题（3 active + 1 needs_review）；OCR runner upload-artifact 连续超时 infra flake。
