...
- `composeTaskCatalog()` 只把每个 entry 的精确 `definition` 投影成冻结的 53-definition runtime map。`src/ai/registry.ts` 再加两个 Copilot dispatch overlay，作为旧 runner 的 compatibility projection；它不是 semantic source。
- `pnpm audit:task-census` 守住 53 registered / 52 statically invoked / 1 explicit compatibility（`AttributionTask`），并检查 manifest/legacy handler reachability 与 `ai_task_runs.task_kind` run-log path。
