...
- **Server 端** (`app/api/ai/[task]` + `src/server/ai/`)：查注册表，走 `@anthropic-ai/claude-agent-sdk` runner；需要 tool 时注入 `allowedTools + mcpServers`。
...
3. 如需 tool calling，把领域工具加到 Domain Tool Registry，并在 task `allowedTools` 里声明
...
产品内 tool 的领域设计见 [Agent Context Tools Design](../../docs/superpowers/specs/2026-05-17-agent-context-tools-design.md)。核心原则：
