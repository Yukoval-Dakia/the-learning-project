...
function buildQuizGenPrompt(profile: SubjectProfile): string {
  return `你是${profile.displayName}出题人，用联网检索来的**素材**写**原创**练习题。输入 { trigger: 'knowledge'|'learning_item'|'manual', ref: { id, name, ... }, knowledge_context, count, few_shot_examples_md?, requested_generation_method?: 'material_grounded'|'closed_book', requested_kind?: string, objective_only?: boolean } —— ref 是触发出题的知识点 / 学习项，count 是期望题数（默认 3）。few_shot_examples_md（若有）是已入库的优质范例，**仅供参考其结构与设问风格，禁止照抄题面**。requested_generation_method 是上游找题次序**指定**的出题方式：出现时**必须**用该方式（material_grounded=据真实素材出题，必须拉真原文并填顶层 material；closed_book=凭已有知识闭卷出题，不强制检索素材）——不要自作主张换成别的方式；缺省时按下面的规则自行选择。objective_only=true 时 requested_kind 是硬约束，每一道题的 kind 都必须与它一致；否则 requested_kind 只是结构提示：理解它代表的答案类型（受限答案、关键词、开放语义或分步推导）与题面结构，优先据此命题，但素材天然支持更合适的结构时可输出实际结构对应的 kind。下游会对每题独立质检。
...
function buildQuestionAuthorPrompt(profile: SubjectProfile): string {
  return `你是${profile.displayName}出题作者，一次只写**恰好一道**原创题。输入 { seed_mode: 'knowledge'|'material', knowledge_context: [{ id, name }], requested_kind?, objective_only?: boolean, requested_difficulty?, material?: { body_md, title? } } —— knowledge_context 是这道题要考查的知识点（id 是你**唯一**能写进 knowledge_ids 的 id）；seed_mode='material' 时 material.body_md 是用户给的命题素材原文，题目必须**据这份素材**出。objective_only=true 时 requested_kind 是硬约束；否则 requested_kind 是答案类型与题面结构指导。
科目上下文：${profile.displayName}。${profile.languageStyle}
证据要求：${profile.grounding.requirement}
不确定性策略：${profile.grounding.uncertaintyPolicy}${rubricGuidanceSection(profile)}
...
function buildSourcingPrompt(profile: SubjectProfile): string {
  return `你是${profile.displayName}题源检索员。任务：根据输入的学科 + 考点/题型 + 数量，**联网检索现成的练习题**，把每道题抽取并结构化为 SourcedQuestion。输入 { subject, knowledge_context, kinds?, objective_only?: boolean, count, whitelist } —— count 是期望题数，whitelist 是可信题源域名列表（可能为空）。objective_only=true 时 kinds 中指定的 kind（即 requested_kind）是硬约束，每道文本题的 kind 都必须与它一致；否则 kinds 是检索时的答案类型与题面结构指导，不要求整批输出复刻同一个 kind 字符串，应忠实抽取每道题的实际结构。
科目上下文：${profile.displayName}。${profile.languageStyle}
证据要求：${profile.grounding.requirement}
不确定性策略：${profile.grounding.uncertaintyPolicy}${rubricGuidanceSection(profile)}
