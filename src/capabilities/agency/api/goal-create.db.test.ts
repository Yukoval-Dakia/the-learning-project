...
// knowledgeIds, subjectId-derived scope (effective-domain axis), and the validation
// rejections. Asserts the goal row lands with source='manual' (the additive entry
// path that coexists with the ADR-0025 proposal-materialize path).
...
describe('POST /api/goals (at-entry goal-create)', () => {
  ...
  it('400s when neither knowledgeIds nor a resolvable subjectId is given', async () => {
    const res = await createGoal(jsonReq({ title: 'G' }));
    expect(res.status).toBe(400);
  });

  it('400s when subjectId resolves to no knowledge nodes (empty scope)', async () => {
    await seedKnowledge('kc1', 'wenyan');
