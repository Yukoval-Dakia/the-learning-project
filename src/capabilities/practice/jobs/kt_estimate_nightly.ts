...
//   「该题有 ≥1 条客观二元作答事件（outcome IN ('success','failure')）」。三条都满足才入选——
//   无硬轨行 → applyKtEstimate no-op（不浪费）；draft → 不该被标定（belt-and-suspenders）；
//   无序列 → estimateBkt 空序列只回吐先验（写 kt_json 也只是 prior-echo，但仍 skip 省开销，
//   且语义上「从未作答的题」不该有 KT 画像，与「prior-echo 是预期值」不冲突——这是 prefilter
//   省工，不是信息门）。partial outcome 不算二元（与硬轨 difficulty 标签同纪律，见序列读取）。
...
export async function runKtEstimateNightly(
  ...
  // (pg-boss retries). 候选 = 有硬轨 item_calibration 行 + question 非 draft + 有 ≥1 条客观二元
  // 作答事件。INNER JOIN item_calibration (track='hard') 保证硬轨行存在；EXISTS 子查询保证有
  // 序列；JOIN question 排除 draft（G5）。DISTINCT 题（一题一行硬轨 → 已唯一，但 EXISTS 不放大）。
  const candidates = await db
    ...
    .where(
      and(
        ...
        // 有 ≥1 条客观二元作答事件（无序列 → estimateBkt 只回吐先验 → prefilter skip 省开销）。
        sql`EXISTS (
