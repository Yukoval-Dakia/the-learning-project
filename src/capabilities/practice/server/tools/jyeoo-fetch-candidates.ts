...
// producer 内容是付费账号资源——治理经 jyeoo-budget 日租约（fetch 核 pre-flight 读取并把
// session_max 裁进余额），不走 LLM 预算。
//
// Canary：由 fetch 核（runJyeooFetchCandidates）单写——每次运行（含失败）恰好一条
// action='experimental:jyeoo_fetch' 事件（预算账本 + 漏斗观测面）。YUK-988 E3 P1-3
// 起从本工具层移入核：executor/脚本直调核时观测/预算归账无法被绕过；工具层不再写。
...
export const jyeooFetchCandidatesTool: DomainTool<Input, Output> = {
  ...
  outputSchema,
  costClass: 'local',
  // 无 safeHandoff：staged 资产 + 核内 canary 事件是 db 写，registry 只允许 idempotent read 远程
  // handoff——本工具固定 in-process 执行（E1 也无远程调用方）。
  mirrorEvent: 'when_causal',
  async execute(ctx, input) {
    // Canary 由核单写（见模块头）——本层只透传归因 ctx，保证 agent 调用与 executor
    // 直调核产生同一条 canary（含 session/task/causal 归因）。
