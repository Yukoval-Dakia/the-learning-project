...
// kind reading vs choice — YUK-386: the kind filter runs in answer-class space, so the
// contrast row must sit in a different CLASS (exact) to be excluded from a semantic
// request; a draft, a sub-floor difficulty, a composite parent) and
...
describe('queryExistingPool ↔ poolFetch selection equivalence (YUK-398 inc-2)', () => {
  it('selects the spec-ordered ids across tier / kind / draft / difficulty / composite axes', async () => {
    ...
    // a 'choice' (exact-class) row — excluded for a semantic-class reading request,
    // included for an exact-class one. (A 'computation' row would NOT be excluded:
    // it shares the semantic class with 'reading' — YUK-386 answer-class filter.)
    await seedQuestion({
      id: 'c-gen',
      knowledgeId: 'kc',
      kind: 'choice',
    ...
    // tier-4 generated by created_at asc. Draft + other-KC + exact-class choice excluded.
    ...
    // ── 4) choice (exact-class) request — only the exact-class row matches ───────
    const choice = await runSourcingSequence({
      db,
      knowledgeId: 'kc',
      count: 99,
      kind: 'choice',
    ...
    expect(choice.existing.map((h) => h.question_id)).toEqual(['c-gen']);
  });
