...
// YUK-863 / F3.2 — Copilot capability owns these five TaskDefinitions.

const CopilotDispatchDecisionSchema = z.discriminatedUnion('mode', [
  z
    .object({
...
const CopilotEvidenceSourceRefSchema = z
  .object({
    call_index: z
      .number()
      .int()
...
const CopilotEvidenceReviewOutputSchema = z
  .object({
...
const CopilotEvidenceVerificationOutputSchema = z
  .object({
    protocol_version: z.literal(1),
    reply_checks: z
      .array(
...
export const copilotTaskSpecs: Record<string, TaskDefinition> = {
  ProfileCriticTask: {
...
{"mode":"durable","reason":"multi_step_research|multi_artifact_work|broad_batch_work"}`,
  ...
  CopilotEvidenceReviewTask: {
    ...
    defaultProvider: 'xiaomi',
    defaultModel: 'mimo-v2.5-pro',
    budget: { ...DEFAULT_TASK_BUDGET, maxIterations: 24, timeout: 120_000 },
    needsToolCall: true,
    isMultimodal: false,
  ...
  CopilotTask: {
    ...
    defaultProvider: 'xiaomi',
    defaultModel: 'mimo-v2.5-pro',
    budget: { ...DEFAULT_TASK_BUDGET, maxIterations: 6, timeout: 60_000 },
    needsToolCall: true,
    isMultimodal: false,
    allowedTools: [],
    prompt: {
