...
> D14 单人格对话面：自由对话 + chip 直触 SSE 流、turns 重放、今日摘要、教学 accept-chip。工具面经 `copilotTools` 贡献制聚合自各 capability 包。

## WHERE TO LOOK
| 文件 | 职责 |
|------|------|
| `manifest.ts` | 4 条 API 路由 + `copilot_run` job + 5 个自有 copilot tools + 4 event actions |
