...
describe('Notes handoff ownership boundary', () => {
  ...
  it('removes the legacy onReady callback and agency best-effort send', async () => {
    const [generateSource, applierSource, agencyAcceptSource] = await Promise.all([
      ...
      readFile(new URL('../agency/server/proposal-accept-applier.ts', import.meta.url), 'utf8'),
    ]);
    ...
    expect(agencyAcceptSource).toContain('dispatchNoteGeneration');
  });
