// YUK-697 — shared tier-2 web_sourced draft INSERT.
//
...
// pre-INSERT exact/near dedup) and AFTER it (writeVerifyDispatchIntent, per-question
// bookkeeping); the helper only builds the row + handles the ON CONFLICT race.
...
import { findExactQuestionDuplicate } from '@/server/quiz/content-fingerprint';
...
export interface InsertSourcedDraftInput {
  id: string;
  q: SourcedQuestionT;
  /** Resolved, existence-checked knowledge ids for the draft (caller owns resolution). */
  knowledgeIds: string[];
...
export type InsertSourcedDraftResult =
  ...
  | { status: 'raced_duplicate'; existingId: string };

/**
 * INSERT one tier-2 web_sourced draft (draft_status='draft'). Returns the built
 * difficulty evidence + supply trace on success, or the existing row id when a concurrent
 * insert won the canonical-hash race. Throws only when an ON CONFLICT fires but no
 * duplicate can be found (an unrelated conflict — a real invariant violation).
 */
export async function insertSourcedDraft(
  ...
  const { canonicalContentHash } = input;
  ...
  const inserted = await tx
    ...
    // Scope the arbiter to the canonical-hash partial unique index (WHERE ... IS NOT NULL) so
    // a bare conflict (e.g. the PK) can't be misread as a hash collision. The `where`
  ...
  if (inserted.length === 0) {
    ...
    return { status: 'raced_duplicate', existingId: raced.id };
  }
