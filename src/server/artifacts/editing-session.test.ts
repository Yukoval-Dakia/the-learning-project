...
describe('isArtifactIdle', () => {
  it('treats an artifact with no recorded session as idle', () => {
    expect(isArtifactIdle('art_unknown', T0)).toBe(true);
  });

  it('returns false while a fresh editing heartbeat is within the timeout window', () => {
    recordEditingHeartbeat({ artifactId: 'art_1', status: 'editing', now: T0 });
    expect(isArtifactIdle('art_1', at(EDITING_HEARTBEAT_TIMEOUT_MS - 1))).toBe(false);
  });

  it('auto-transitions an editing artifact to idle once the heartbeat times out', () => {
    recordEditingHeartbeat({ artifactId: 'art_1', status: 'editing', now: T0 });
    expect(isArtifactIdle('art_1', at(EDITING_HEARTBEAT_TIMEOUT_MS + 1))).toBe(true);
    // The transition is sticky: the snapshot now reads idle.
    expect(getEditingSessionSnapshot('art_1')?.status).toBe('idle');
  });

  it('returns true immediately for an explicitly idle heartbeat', () => {
    recordEditingHeartbeat({ artifactId: 'art_1', status: 'idle', now: T0 });
    expect(isArtifactIdle('art_1', T0)).toBe(true);
  });
});

describe('recordEditingHeartbeat', () => {
  it('stamps editingStartedAt only on the first editing heartbeat and clears it on idle', () => {
    recordEditingHeartbeat({ artifactId: 'art_1', status: 'editing', now: T0 });
    ...
    recordEditingHeartbeat({ artifactId: 'art_1', status: 'editing', now: at(5_000) });
    expect(isArtifactIdle('art_1', at(5_000))).toBe(false);

    recordEditingHeartbeat({ artifactId: 'art_1', status: 'idle', now: at(6_000) });
    expect(isArtifactIdle('art_1', at(6_000))).toBe(true);
  });
});
...
describe('enqueueOrApplyNoteRefinePatch', () => {
  ...
  it('defers the patch while the artifact is actively being edited', async () => {
    ...
    expect(getEditingSessionSnapshot('art_1')?.pending_patches).toBe(1);
  });
...
describe('markArtifactIdleAndFlush', () => {
  it('flushes queued patches in order and reports the flushed count', async () => {
    ...
    expect(getEditingSessionSnapshot('art_1')?.pending_patches).toBe(0);
    expect(isArtifactIdle('art_1', at(3_000))).toBe(true);
  });
...
describe('getEditingSessionSnapshot', () => {
  it('returns null for an artifact that has never had a session', () => {
    expect(getEditingSessionSnapshot('art_none')).toBeNull();
  });

  it('exposes status, heartbeat time, and pending count for an active session', async () => {
    recordEditingHeartbeat({ artifactId: 'art_1', status: 'editing', now: T0 });
    await enqueueOrApplyNoteRefinePatch({ db, artifactId: 'art_1', patch, now: at(1_000) });
    const snapshot = getEditingSessionSnapshot('art_1');
