...
// YUK-350 — reuse the exact judge's choice-resolution rules at VERIFY time so the
// deterministic objective structural reject filter asserts the SAME「reference resolves
// to a valid unique choice」property the grade-time judge relies on.
import { resolveChoiceIndices } from '@/core/capability/judges/exact';
...
//
// CORE PRINCIPLE: at verify-time there is NO student answer, and the whole question
// structure (prompt + choices + reference) is co-generated by ONE LLM, so there is NO
// deterministic ground truth for ANSWER CORRECTNESS. Therefore this deterministic check
// can ONLY produce a REJECT signal (malformed structure → reject without burning the LLM);
// it can NEVER produce a PROMOTE. This mirrors the deterministic n-gram copy_safety fold
...
export interface ObjectiveStructuralRejectInput {
  kind: string;
  reference_md: string | null;
  choices_md: string[] | null;
}

// The result can ONLY reject (malformed:true + reason) or fall through (malformed:false).
// There is intentionally NO promote/pass/active field — the filter grants NOTHING.
export type ObjectiveStructuralRejectResult =
  | { malformed: false }
  | { malformed: true; reason: string };

/**
 * Deterministic structural validity check for closed-book OBJECTIVE drafts (choice /
 * true_false). Pure: no LLM, no DB. Returns `{ malformed: false }` for any non-objective
 * kind (fall through) and for a structurally well-formed objective draft; returns
 * `{ malformed: true, reason }` only when the objective structure is broken. It can NEVER
 * signal a promotion.
 */
export function runObjectiveStructuralRejectFilter(
  input: ObjectiveStructuralRejectInput,
): ObjectiveStructuralRejectResult {
  // Guard: only choice / true_false enter the filter. Everything else falls through
  // (prose / translation / short_answer / essay / reading / derivation / fill_blank).
  ...
  // grade-time). This is NOT reference-vs-reference: the reference is resolved AGAINST the
  // choices array, and the asserted property is "single, in-range, resolvable".
  const referenceIdx = resolveChoiceIndices(reference, choices);
...
export async function runQuizVerify(params: RunQuizVerifyParams): Promise<RunQuizVerifyResult> {
  ...
  // YUK-350 — DETERMINISTIC OBJECTIVE STRUCTURAL REJECT FILTER (runs BEFORE the LLM).
  // For closed-book objective kinds (choice / true_false) a malformed STRUCTURE (a
  // reference that resolves to no / non-unique choice, < 2 or duplicate choices, a
  // non-boolean true/false reference) can be detected deterministically — there is no
  // point spending an LLM verify on a draft that is structurally broken. On a malformed
  // draft we SHORT-CIRCUIT: persist a verify event with a structure_completeness=fail
  // axis (overall=needs_review, NOT pass, NOT active), leave draft_status='draft', enroll
  // NO FSRS, and RETURN before `runTaskFn` ever runs (the LLM-saving win). A structurally
  // VALID objective draft falls through unchanged — the filter grants NOTHING; the LLM
  // verify path below still runs its closed-book axes and the existing promote gate
  // decides. Non-objective kinds never enter the filter (malformed:false → fall through).
  const structuralCheck = runObjectiveStructuralRejectFilter({
    kind: row.kind,
    reference_md: row.reference_md,
    choices_md: row.choices_md,
  });
  if (structuralCheck.malformed) {
    const now = new Date();
    const verifyEventId = createId();
    // Project onto the unified verify contract. overall='needs_review' (NOT pass), promote
    // is hardcoded FALSE — the deterministic filter can ONLY reject, never promote. The
    // single axis is the deterministic structural verdict.
    const summaryMd = `结构复核未通过（确定性）：${structuralCheck.reason}`;
    const unified = toUnifiedVerifyResult({
      ...
      confidence: 1,
      checks: [{ axis_name: 'structure_completeness', verdict: 'fail' }],
    });

    const verification: QuizGenVerificationT = {
      status: 'needs_review',
      summary: summaryMd,
      verified_by: { by: 'ai', task_kind: 'quiz_verify_structural' },
    };
    const updatedMeta: QuizGenMetadataT = { ...meta, verification };
    const newMetadata = { ...metadataRaw, quiz_gen: updatedMeta };

    await db.transaction(async (tx) => {
      // Stay draft — NEVER promote, NEVER enroll FSRS. Only persist the verification
      // block so the row carries the structural-reject reason.
      await tx
        .update(question)
      ...
      await writeEvent(tx, {
        ...
        // 'partial' = a verdict was produced but did not promote (mirrors the needs_review
        // success path). NOT 'error' (that is the transient catch-bottom, retriable) and
        // NOT 'success' (no promotion). A terminal partial verdict makes the idempotency
        // guard treat this as already-verified, so a structural reject is not re-run.
        outcome: 'partial',
        payload: {
          question_id: questionId,
          ...unified,
          // Deterministic structural reject marker (audit trail) — distinguishes this
          // LLM-free reject from an LLM verdict.
          structural_reject: { reason: structuralCheck.reason, kind: row.kind },
          source_tier: tier,
          promoted: false,
    ...
    // U8 / AF §4 — same pool-gap hint as the LLM non-promote path: a draft that did not
    // enter the pool leaves its knowledge point(s) without a usable question. Best-effort.
    const poolGapRefs = (
    ...
    try {
      await writeAgentNote(db, {
        ...
        summary_md: `Generated question ${questionId} failed the deterministic structural check (${structuralCheck.reason}); its knowledge point(s) may still lack a usable question.`,
        confidence: 1,
      ...
      console.error(
        '[quiz_verify] structural-reject leave_agent_note failed (non-fatal) for',
        questionId,
        noteErr,
      );
    }

    return { status: 'needs_review', overall: 'needs_review' };
  }
