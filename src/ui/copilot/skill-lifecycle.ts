...
//
// The Dock clears its active skill context on a teaching `end` turn. But quiz +
// solve are ONE-SHOT skills: they return NO terminal `skill_turn` (chat.ts quiz /
// solve branches), so the `end`-turn clear never fires for them. Without this guard
// every follow-up free-form message after a quiz/solve would keep re-sending the
// stale skill_context (the YUK-213 F2 one-shot-stuck bug).
//
...
export const ONE_SHOT_SKILLS = new Set(['quiz', 'solve']);
