...
describe('live SoT — shipped shared SKILL.md packs resolve against the real tree', () => {
  ...
  it('keeps the dialogue methodology compact and focused on choosing useful reads', () => {
    const content = readFileSync(
      join(process.cwd(), 'src/subjects/_shared/skills/copilot/SKILL.md'),
      'utf-8',
    );
    expect(content.length).toBeLessThan(5000);
    expect(content).toContain('先选择直接回答学习者问题的语义读取工具');
    expect(content).toContain('已经核验的共同点、分叉与具体缺段');
  });
});
