...
- 按 logical subject 重建跨阶段链时，先用 `subjectId` **不带 `subjectKind`** 查询；不同阶段可能从 `knowledge` 变成 `mind_model`。若 `query_events.subject_scope.all_subject_kinds_included=false`，该结果没有覆盖同 ID 的其他 kind，必须去掉 kind 重查，不能据此宣布后续阶段不存在。
