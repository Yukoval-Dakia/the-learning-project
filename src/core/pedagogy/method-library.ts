...
// 真正的消费点（B5 verify contract / panel-SELECT）仍未接线。这是刻意的 deferred 终态，不是遗漏。
//
...
// 制造一条 STALE 发现（为了让审计变绿而弄脏审计）。等接线时再按 A5 dark-ship 纪律在**真正的消费点**
// 登记闸门 flag。审计遇到本模块请引 ADR-0050 §(c)，不要重开死码发现。
